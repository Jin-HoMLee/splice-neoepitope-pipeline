...
- **Environment:** add `mhcflurry` to `research/requirements.txt` so all of #708 runs in `research/.venv` (one env, fully solo; per-clone setup needed on this clone anyway).
