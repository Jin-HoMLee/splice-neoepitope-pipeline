...
- **R `cir` oracle (REVISED post-implementation — now live, not a proxy):** the initial fixtures were hand-computed (R was unavailable at build time) and the test asserted exact knot-array equality. Resolved by installing **R 4.6.0 + `cir` 2.5.1** and regenerating `tests/fixtures/cir_fixtures.json` live (versions recorded in the fixture). Two findings: (1) `cir` 2.5.1's `cirPAVA(full=TRUE)` returns the centred knots in the `$shrinkage` frame (the generator was updated accordingly); (2) **R preserves boundary-anchor knots inside a collapsed level-set that the compact port drops** — the hand-computed fixtures shared the port's omission and so masked the gap (a self-validation trap). Verified *not* a bug: the calibrator consumes knots via `np.interp` with endpoint-clipping (`calibrator.py` `transform`), under which both knot sets define the **identical** function (max|Δ| = 0 over a dense grid). The test was therefore over-specified; `test_centered_isotonic.py` now asserts **functional equivalence** (interpolated curve vs the live oracle over a 1001-point grid) rather than an arbitrary knot encoding, and two genuine PAVA-pooling cases (non-monotone `y`) were added since the original four were all pre-monotone. Production `calibrator.py` is unchanged. `generate_cir_fixtures.R` documents the R/`cir` version; no standing R dependency in CI.
