...
### The analysis may be a tested tool, not a notebook - and where it must NOT live

The layout above shows `notebook.ipynb`, but an experiment whose analysis is **re-run across issues** (rather than explored once) should ship it as a **tested CLI tool co-located in the experiment dir**, with a `tests/` subfolder and a `conftest.py` putting the experiment dir on `sys.path`. Precedents: `issue_547_immunogenicity_calibration/` (`calibrator.py`, `reliability.py`, `prg_metric.py` + `tests/`) and `issue_919_nh_uniqueness_filter/` (`junction_repeat_overlap.py` + `tests/`). Rationale, per [Cookiecutter Data Science](https://cookiecutter-data-science.drivendata.org/opinions/): source code "is more portable, can be tested more easily, and is easier to code review", so the direction of travel is exploration → **refactor the good parts into source code**. A notebook is right for a one-shot exploration; a tool whose numbers are the evidence base for a production default is not that.

**These tests run in CI** (`research-pytest` job in `.github/workflows/tests.yml`). Add each new experiment `tests/` dir to that job explicitly - `pytest research/` is not used because it would descend into the gitignored per-clone `research/.venv` and collect third-party tests. The job carries the scientific stack from `research/requirements.txt`; the pytest-only `ci-tools-pytest` job deliberately does not. (The research suites were CI-invisible until [Issue #1117](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1117) - the same regression class as `tools/project_map` in [#713](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/713) and `scripts/tests` in [#901](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/901): a test home nobody wired into a job rots silently.)

**Do not put experiment apparatus in `workflow/scripts/`.** That directory is for Python a Snakemake rule invokes; a file there implies a rule runs it. Analysis code that only a human runs belongs in the experiment dir, however "pipeline-shaped" it looks. The converse also holds: a rule that *fetches a reference input* into `references/` is production and belongs in `workflow/rules/` even if it sits outside the default DAG (e.g. `download_rmsk_chrom`, a sibling of `download_vdjdb_release`).
...
- **10-100 MB:** check into git **only if** offline-regenerable from committed inputs, and commit the regenerator script alongside. If it is **not** offline-regenerable - e.g. derived from a paid or quota-limited external API with no committed regenerator - keep it out of git regardless of the band, and give it a **remote mirror on the same R2 store as the > 100 MB band (`experiments/<issue>/`) + a `data_manifest.yaml` entry** (paths + checksums + fetch commands) so a fresh clone can fetch it rather than it living only on the producing machine.
