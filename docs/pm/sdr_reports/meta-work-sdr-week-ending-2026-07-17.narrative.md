...
Per-role delivered: developer 28, scientist 13, PM 12. Full itemized list in the Inventory appendix.

## Descoped (closed NOT_PLANNED)

Both closed on 2026-07-14 by supersession - each subject was overtaken by other landed work:
...
**Process note:** neither descope carried a human closing comment - only the closure-audit bot fired (flagging unticked ACs). A NOT_PLANNED close should carry a one-line closing rationale + supersession pointer *before* the close (closure ritual). Folded into the retrospective below.
...
**One process slip to correct:** both NOT_PLANNED descopes ([#1158](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1158), [#1095](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1095)) closed with no human closing comment. The closure-ritual rule (a direct close posts its rationale first) held for merging-PR closes but slipped on these two direct closes. Low-stakes here (both were clean supersessions), but worth a reminder rather than a mechanism.
