...
| `gh` | 2.65.0 (re-confirmed 2026-07-04, [#941](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/941)), authed via `$GH_TOKEN` - fine-grained PAT, **repo + project** scope | can push, open PRs, comment, **and** move project-board fields. **But 2.65.0 < 2.94.0**, so the native dependency **`--json` client fields** (`gh issue view --json blockedBy` / `blocking` / `parent` / `subIssues`) are unavailable in-sandbox - gh validates `--json` names against a built-in per-version list. This does **not** bar native blocked data cross-env: a raw `gh api graphql` passthrough is server-evaluated (the `blockedBy` edge is GA since 2025-08-21) and is **not** client-version-gated, which is exactly how `scripts/pm/scan_prose_deps.py` reads the graph. So the rule is *use the `gh api graphql` passthrough form, not the `--json` client field* - not *avoid native data entirely*. No committed script uses the `--json` client field today (`scripts/audit_and_merge.sh` has no blocked read at all; the "morning blocked-graph hygiene check" is a manual PM query); keep bare `is:blocked` / GraphQL for that manual read ([#824](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/824), [#942](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/942)). An in-sandbox `gh api graphql` blockedBy probe would confirm 2.65.0 passthrough end-to-end if a cross-env script ever needs it. |
