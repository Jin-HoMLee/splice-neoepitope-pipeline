...
> **Status: established 2026-06-03.** Validated by the overnight one-shot batch — #632 pilot ([PR #647](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/647)) + env-probe smoke test; #641 ([PR #648](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/648)) confirmed the corrected `pip install`, the stray-closer guard, and the env-ferry, and surfaced the **network egress allowlist**; #375 ([PR #649](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/649)) validated the heavier `requirements-test.txt` install (pandas/biopython/numpy) **and** the conda-absent → **`snakemake -n` deferred-to-CI** pattern (#647/#648/#649 all CI-green incl. `pipeline-snakemake-dry-run`); #435 was the 4th. Re-probe the sandbox table if the platform image changes.
...
| git identity | preset to `Claude <noreply@anthropic.com>` | commit author is already Claude (so the `Co-Authored-By: Claude …` trailer is technically redundant — pick one attribution) |
...
5. **Branch `claude/issue-NNN-…` off `origin/main`** (plain `git switch -c`, *not* `gh issue develop`); open a **draft** PR; **never merge, never force-push**.
...
9. **`allowed_tools`:** for a first/uncertain task, **bare `Bash`** + file tools avoids the un-allow-listed-command hang.
