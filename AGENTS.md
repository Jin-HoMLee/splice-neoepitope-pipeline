...
**Rendered active-arc slate ([Issue #759](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/759)).** `scripts/pm/arc_taxonomy.tsv` is the source of truth but is invisible day-to-day, so the **board #9 README** (the project description) carries a human-readable mirror of the slate — every `arc:<slug>` grouped by `active` / `next` / `later` phase. That pin is **hand-maintained**: refresh it at each arc review (PM-coordinated) whenever an arc's phase or membership changes. It is a *surfaced view*, never the source of truth — the TSV stays canonical, the README is the glance-able render. (Decided 2026-06-26 over a live renderer / generated docs artifact: arc-phase changes only happen at periodic PM-coordinated arc reviews, so the manual refresh folds into a ritual already touching these labels.)
