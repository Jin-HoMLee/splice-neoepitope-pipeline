...
- **`run_cloud_gpu.sh` defaults to SPOT provisioning + `pd-balanced` boot disk** ([Issue #833](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/833)), to trim cost on the GCP runway. Both are applied **only when CREATING** the VM — provisioning model and boot-disk type are **immutable post-create**, so the existing production `neoepitope-pipeline` (created on-demand + `pd-ssd`) keeps those until it is deleted + recreated. Pass `--on-demand` to force STANDARD (e.g. when SPOT P100 capacity is unavailable — likely in a constrained zone, since spot is reclaimed first). On SPOT, a mid-run preemption stops the VM (`--instance-termination-action=STOP` → TERMINATED, boot disk + partial results preserved); the completion poller detects TERMINATED and exits with a resume hint — **re-run the same command to resume** via `snakemake --rerun-incomplete`.
