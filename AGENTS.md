...
> **⚠️ Newline blindness is in the HOOK's own matcher, NOT in the `if:` gate ([Issue #1142](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1142), 2026-07-13).** A hook that tokenizes the raw command with `shlex` and looks for its trigger *at a command start* is **silently dead on every multi-line command**, because **a newline is a command separator in shell but not in `shlex`**. After `cat > body.md <<'EOF' … EOF`, or simply on the second line of a two-line command, the trigger sits after an ordinary word rather than a separator, the match fails, and the hook **allows** - no error, no fire-log line, just nothing. This killed `post_gh_pr_create` on the dominant PR shape ([Issue #1130](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1130)) and, until #1142, also `check_board_query_pagination` and `check_gh_issue_develop_parent`.
>
> **The fix is `_shell_parse.normalize_command(cmd)` before tokenizing** (strips heredoc bodies, turns *unquoted* newlines into separators). It **strengthens** the anti-false-positive property rather than weakening it: a heredoc body is *removed* instead of tokenized, so a comment body that merely *discusses* `gh pr create` still cannot match. **Any new `gh`/`git`-matching hook MUST call it** - shipping without it is shipping a hook that is born dead on the shape we actually use.
>
