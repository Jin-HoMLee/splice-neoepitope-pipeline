...
The Ready queue is guarded against *starvation* (per-role floor-5 / cap-18, [Issue #754](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/754); full rule in shared memory `feedback_ready_queue_floor_gate.md` / `feedback_board_hygiene.md`) but had **no guard against `In progress` *overload*** until [Issue #690](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/690) sub-question D (2026-06-19).
