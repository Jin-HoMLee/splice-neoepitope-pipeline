...
**Known limit of the content-guard tier - the `Bash`-write blind spot is accepted, not closed ([Issue #1197](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1197), ratified 2026-07-17).** The no-em-dash guard (`check_no_emdash.py`) is bound to the **`Edit`/`Write`** matchers and inspects the text those tools write, so content written through a `Bash` heredoc - or `printf`/`echo`/`tee`/`python -c`/redirection-from-a-variable - never passes through the matcher and is invisible to it. **Scope precisely** (this is what the original #1197 framing over-broadened): the em-dash guard is the *only* `Edit`/`Write`-bound content guard today. The `@-claude`-mention guard is `Bash`-bound (`if: "Bash(gh *)"`) and the `Created by:` comment-footer guard ([#1094](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1094), now built - see its subsection below) likewise guards a `gh` **command body**, not a file write - so those two are the *command-string* family whose blind spot is the non-convergence documented in the preceding paragraph ([#1142](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1142)), a different gap from this `Edit`/`Write`-vs-`Bash`-write one. We deliberately do **not** extend the `Edit`/`Write` content matcher to parse heredoc bodies: that is a denylist widen (CWE-184) over an unbounded set of `Bash` write paths, so it never converges and manufactures false confidence ("Bash writes are guarded" when only heredocs would be) - the same non-convergence the [#1150](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1150) posture names for the command-string guards. The gap is accepted as the known limit of the best-effort tier and backstopped by review; the threat model is accidental self-bypass, not an adversary. The **convergent** fix, if we ever want a real guarantee, is not a better matcher but an **effect-level** content check that inspects the resulting file content/diff after any write regardless of tool (a PostToolUse-on-write or a Stop-hook diff scan) - tracked, deferred, in [Issue #1227](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1227). This is the content-guard instance of the rung-4 principle below: real coverage lives at the effect boundary, not in the string matcher.
...
**Design (composes with the [#1197](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1197) accept-the-hole decision):** it DETECTS the comment command on the *normalized* text (so a `gh comment` after a heredoc still matches - the #1130/#1142 lesson) but INSPECTS the body, and it **denies only a body it can actually see**: an inline `--body`, or a heredoc feeding `--body-file -` (the body is in the raw command string). A real `--body-file <path>` is **not** inspectable, so it fails **open** (allow) - denying a compliant file-bodied comment would be a false positive. A `--body-file -` with **more than one** heredoc in the command is ambiguous (the body is the `gh`-line heredoc, but an earlier scratch heredoc may precede it), so it too fails **open** rather than risk reading the wrong body ([PR #1229](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1229) finding 1). This is a `Bash`-`gh`-command guard, not an `Edit`/`Write` content guard, so its residual blind spots (a `gh api ... /comments` PATCH, a real `--body-file`, an ambiguous multi-heredoc `--body-file -`) are the accepted best-effort limit per #1197, not a defect.
