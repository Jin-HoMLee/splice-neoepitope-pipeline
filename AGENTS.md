...
The Ready queue is guarded against *starvation* by a **demand-aware per-role replenishment trigger** (floor-3 / cap-12 in `check_ready_queue.sh`, [Issue #754](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/754) + [Issue #902](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/902) facet 1; full rule in shared memory `feedback_ready_queue_floor_gate.md` / `feedback_board_hygiene.md`) but had **no guard against `In progress` *overload*** until [Issue #690](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/690) sub-question D (2026-06-19).

**Replenishment trigger ([Issue #902](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/902) facet 1, 2026-06-30) - demand-aware, not a fixed quota.** The per-role Ready floor is a *replenishment trigger*, and it fires only when a role is **actually consuming** - i.e. it has `>= 1` item In progress **and** its Ready count is below the floor (default 3, one In-progress WIP slate). A role idle on a quiet/post-ship board (0 In progress) does **not** nag, even at 0 Ready: with no demand drawing the buffer down, holding thin Ready is the correct late-commitment posture (Last Responsible Moment), not a starvation signal. The total cap (default 12 = floor x 3 + 3 headroom) is a demand-independent soft WIP limit on the Ready buffer. **Why demand-aware** (web-grounded): a Kanban replenishment "minimum order point" should be sized from consumption x lead time and replenished demand-pull, never as a guessed fixed number - the buffer exists only to keep the downstream (In progress) from starving, so with no demand there is nothing to starve. We have **no flow data yet**, so the faithful translation is to gate the trigger on actual draw-down and keep the number to one WIP slate, then retune from flow data later. This replaced the original #754 fixed floor-5/cap-18, which fired false `[REPLENISH]` pressure on quiet post-ship boards (the 2026-06-29 over-commitment + 2026-06-30 all-roles-fire incidents). The floor is never a force-commit quota: below floor for a consuming role, pull that role's highest-priority DoR-ready Backlog; if fewer than the floor are DoR-ready, commit what's ready and flag a grooming gap.
