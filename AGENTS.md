...
**Why** (web-grounded 2026-06-30): this is the canonical Kanban answer. David J. Anderson - **["Banish Priority and Prioritization"](https://djaa.com/ban-priority-and-prioritization/)** - argues priority is a *proxy variable* masking real risk (cost of delay, skills, dependencies), and that a backlog clustering at one band is the documented failure mode; the prescription is **risk profiles / classes of service** in place of priority, **selection / scheduling / replenishment** (dynamic pull) in place of prioritization, and to **"maintain backlogs as unordered lists."** **What we rejected:** *forced-ranking a per-role top-N* (the explicit anti-pattern - a manual total-order on a deep backlog "requires 100% manual intervention" for no flow benefit at our scale), and the *class-of-service **policy** machinery* (expedite WIP limit, reserved capacity, dedicated swimlanes, a full `class:*` family - these govern a contention problem we do not have at 1 human + N personas).

**Correction ([Issue #811](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/811), 2026-07-13).** Facet 3 originally *also* rejected any separate class-of-service surface, on the grounds that "our coarse `P0`-`P3`, read as classes, already carries the same signal". **That claim is false.** It conflates two orthogonal axes:

| Axis | Question | Carried by |
|---|---|---|
| **Urgency** | how costly is delay? | `P0`-`P3` |
| **Arrival** | did it cross the `Backlog -> Ready` commitment act, or barge in mid-session? | the **`unplanned`** label |

A `P2` same-day fix is **unplanned**; a `P1` committed Ready item is **planned**. The band **cannot distinguish them**, so it cannot answer *"what share of delivered work was unplanned"* - which is precisely the question the SDR-driven WIP retune needs answered. Facet 3's own revisit-trigger ("revisit only if classes ever need distinct *policies*") was set on the wrong condition: the **measurement** value alone justifies the marker, independent of any policy. And facet 3 cited Anderson's *Banish Priority* - whose prescription is to replace priority **with** classes of service - then took the banish half while declining the replacement half.

**What landed instead: measurement only.** A single **`unplanned`** label (**arrival, not urgency**; applied **at close**, by the author of the closing PR), plus an SDR throughput split by it - `scripts/pm/milestone_report.py` reports committed vs unplanned and the unplanned percentage (the Kanban *Throughput Breakdown*, reduced to the one split we actually need). Priority stays a coarse class-of-service **hint**; it is **not** an arrival marker. The policy machinery above stays rejected, and is revisited only if the measured unplanned share turns out to be large.
