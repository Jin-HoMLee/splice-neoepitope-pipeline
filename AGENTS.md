...
**Rung 4 - the tier you actually rely on - is the entry point, not a better matcher.** Anything we want to genuinely depend on routes to an OS-or-server-enforced choke point that never parses a shell, so no un-enumerated shell construct can evade it: `scripts/audit_and_merge.sh` **is** the merge entry point (there is no command string to misread), and GitHub branch protection is a server-side control on `main`. This is the canonical three-layer model (Layer 1 OS/kernel, Layer 2 config/tool-enforced, Layer 3 model instruction), with the rungs you can trust sitting *above* a PreToolUse hook. "I want to actually rely on this" now routes to rung 4, not to another string-matching hook at rung 3.
