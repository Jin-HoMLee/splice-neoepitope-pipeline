...
Every research-decision Issue — a science/methods call that **gates downstream work** but is neither a per-tool evaluation nor a data-analysis experiment — ships a Quarto deck that walks the decision arc: question → options → evidence → decision → caveats. Decks live at `research/decisions/issue_NNN_<short>/slides.qmd`, a fourth sibling next to `research/slides/`, `research/evals/`, and `research/experiments/`. Shared scaffolding at `research/slides/` is referenced via relative paths: `csl: ../../slides/nature.csl` plus the shared reveal.js theme `theme: [simple, ../../slides/custom.scss]`. Unlike the eval/experiment decks, a decision deck **does not extend `_template.qmd`** — it inlines its own YAML front-matter (the difference from the other two sections is by design, not an omission). Bibliography is deck-local (`bibliography: refs.bib`). See [`research/slides/README.md`](research/slides/README.md) for the shared convention (Quarto rationale, render commands, Zotero linkage, install).
