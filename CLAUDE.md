...
### Parent-Status precondition guard ([PreToolUse hook](.claude/settings.json))

Refuses a board Status mutation that advances a **sub-issue** to an *advanced* Status (`In progress` / `Ready for review` / `In review`) while its **native parent** sits in `Backlog` or `No Status`. The hook runs `.claude/hooks/check_parent_status_precondition.py` on stdin-piped PreToolUse JSON and emits a `permissionDecision: deny`. A board Status write is a `gh api graphql … updateProjectV2ItemFieldValue …` mutation carrying an **itemId** (`PVTI_…`) + **singleSelectOptionId**, not an issue number — so the hook parses every mutation block (handling the aliased PR+Issue dual-mutation), keeps only Status-field writes to an advanced target, then resolves each `itemId → issue → issue.parent → parent's project-#9 Status` via one GraphQL query and refuses iff the parent is on board #9 in Backlog/No-Status. It is **read-only** and **fails open** on every uncertain path (not a Status mutation, a non-advanced target → Backlog/Ready/Done, a PR item or standalone issue with no parent, a parent not tracked on board #9, or any `gh` error/timeout). Each deny appends one line to `.claude/hook_fires.jsonl` ([Issue #453](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/453) fire-log infra).

**Why:** the board mirrors a parent's Status from sub-issue progress — *the first sub-issue to start bumps the parent to In progress* (`shared/feedback_parent_sub_issues.md`). Advancing a sub while the parent is still Backlog silently desyncs the board (the epic reads "untouched" while its subs fly; incident #480). The rule is Reference-tier in memory and doesn't load at the Status-mutation moment — rung-3 mechanism escalation ([Issue #499](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/499)). Agent-side companion to the source-agnostic GitHub Action ([Issue #498](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/498)): the hook catches the slip *before* it lands; the Action is a post-hoc safety net for non-agent surfaces.

**Workaround:** bump the parent to In progress first (the legitimate fix — the parent *should* move when the first sub starts), then retry the sub's Status change. Moving the sub to `Backlog`/`Ready`/`Done` is never blocked.
