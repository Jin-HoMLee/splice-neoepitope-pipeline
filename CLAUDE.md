...
- **NVIDIA driver: keep the image's pre-installed `535.x` driver — do NOT install `nvidia-headless-570-server` or any later series.** Image family `common-cu129-ubuntu-2204-nvidia-580` ships `535.288.01` matched with the running kernel's `linux-modules-nvidia-535-server-...` (the `nvidia-580` family-name refers to CUDA toolkit, not driver). 535 < 575 so P100 (SM 6.0) is supported. Upstream Ubuntu (as of 2026-05-27) reduced `nvidia-headless-570-server` to a hard-`Depends:` wrapper for `nvidia-headless-580-server` — installing the 570 metapackage cascades 580 in, and 580 drops Pascal. Also do NOT run `apt-get update` then `apt-get install nvidia-utils-535-server`: archive churns the userspace 535 package past the image's kernel-module version (535.288 → 535.309 observed), creating an irrecoverable kernel-module / userspace mismatch (the `nvidia-kernel-source-535-server` package doesn't ship a usable `dkms.conf`, so DKMS can't rebuild). The image's matched 535 state is self-consistent at boot — `run_cloud_gpu.sh` verifies via `nvidia-smi` smoke test after VM start ([Issue #522](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/522)).
