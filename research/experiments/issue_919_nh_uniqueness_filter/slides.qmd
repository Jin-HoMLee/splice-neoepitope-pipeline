...
**Case 4 is the sting.** The only multimapping a chr22-only index can *see* is chr22-internal paralogy - so that is the only thing the filter can act on here, and it is exactly where the four `IGLV2` losses came from.
...
**The scientific decision is [#1122](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1122) (Scientist):** what *are* these multimapped reads, and is unique-only the right semantic for a pipeline whose target domain is precisely the paralog-rich HLA / IG / TCR loci - the one place the field-standard default may be wrong?

**Sequenced behind it:**

1. **[#1118](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1118)** (P1) - make the two aligners mean the same thing. *Precedes everything else*: the rest tunes a filter whose semantics are currently incoherent.
2. **[#1116](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1116)** - score/rescue instead of gating. Its Portcullis-style Hamming probe is **index-independent**, so it *would* work on the fixture we already have.
...
These were **wrong and are withdrawn** - flagged here because both survived into an Issue, a PR comment, and an earlier version of this deck before being caught:

1. ~~"A chr22-only index converts genome-wide multimappers into false-unique reads."~~ Too coarse. A **diverged** copy maps uniquely and *correctly* genome-wide, and simply goes **unmapped** on a chr22 index. Only **near-identical** families are affected (slide 8).
