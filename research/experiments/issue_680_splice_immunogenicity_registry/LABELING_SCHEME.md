...
> **⚠️ Zhao Table S1 motivated this schema but is not admitted by it.** The first-hand audit ([#1089](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1089)) found its 139 rows carry only *predicted* binding (`avg_rank` / `PHBR_avg` / per-allele affinity), so every row is `label=untested` and **no tier in section 4 fits it**; and the 139 rows resolve to only **103 distinct junctions** (65 rows share a coordinate). Nullable identity is necessary for a coordinate-first source but not sufficient to admit one - the row still needs a legal `(label, tier)`, and none of the section-4 tiers fits an untested prediction (note `tier=candidate` is a *contested positive*, not this). Whether the registry takes a new predicted-only tier is [#1125](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1125). Full audit: [`PROVENANCE.md`](PROVENANCE.md).
