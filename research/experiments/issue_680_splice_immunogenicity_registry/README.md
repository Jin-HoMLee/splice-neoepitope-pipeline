...
- **Zhao 2025 sequence-retrieval** — fetch the main paper PDF / peptide-sequence table to recover the amino-acid strings for the human per-peptide functional hits (pA02-/pA11- IDs), then fold as `functional-scorable` (A\*02:01/A\*11:01/A\*24:02). Sequence-blocked, not gate-blocked.
- **Merlotti single-replicate candidates** (~10 peptides, recorded in PROVENANCE) — re-assess if a stricter or looser inclusion bar is adopted.
- #681 immunopeptidome mining → MS-presented tier (+ the #733 Tier-2 routes above) + decoy negative construction.
