...
- **All 97 current rows are `published-recovered`** - every row in the registry today carries its sequence. The schema change is what makes the *next* fold possible: Zhao 2025 ([#817](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/817)) publishes 139 candidate AS antigens by genomic coordinate with **zero** sequences, and coordinate-native measured true-negatives ([#911](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/911)) are the scarcest reagent in the set.
