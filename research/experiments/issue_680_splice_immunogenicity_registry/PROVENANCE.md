...
Its per-peptide reactivity and peptide sequences sit in its **paywalled main text**, so a **candidate** route to a second hard negative there is an **author data request** (cheaper than wet-lab), tracked in [#1214](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1214) - candidate, not confirmed, because the paper validates *positives*, so whether its panel includes a presented-but-non-reactive peptide is exactly what the request would establish.
This is the **same upstream fetch** as the [#817](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/817) B2MJ776X positive-sequence recovery (line 129) - both are unblocked by obtaining the main text, so batch the request.
