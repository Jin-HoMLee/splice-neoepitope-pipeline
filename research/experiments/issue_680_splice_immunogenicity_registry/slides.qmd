...
But thin is not second-rate. **Our search found no splice-derived antigen that has helped a patient in a trial** - every therapy aimed at one is still preclinical, and the neoantigen treatments that *have* shrunk tumors target mutations, not splicing. So these 82 rows look like the best human evidence the field has (searched 2026-07-11; literature only, no trial-registry sweep - [#1119](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1119)).

## 82 positives are checked by only 9 real negatives {.smaller}
...
## Keying on the peptide locked out a whole class of studies {.smaller}

![](figures/fig_identity_matrix.png){fig-align="center" width="62%"}

Every row used to need the peptide's amino-acid sequence just to exist. But a splice study's native output is the **junction** - where the gene was cut and rejoined - not the peptide. Zhao 2025 [@zhao2025hcc] publishes its candidates (Supplementary Table S1) **by genome location with no sequences**, so the old peptide-keyed schema could not represent them at all. It still adds **zero rows** - the audit found those candidates are *predicted only*, and Zhao's separately validated antigens cannot be joined to them.
