...
2. **Letting a row's "how well is the junction known" grade lag behind a location we recovered later.** The grade reflects what the *study* published; the location column reflects what we hold. Defensible, but the two can disagree, which may surprise a reader.
...
**Where this lives:** `registry.tsv` (97 rows) · `LABELING_SCHEME.md` §7 · `validate_registry.py` + `registry_dedup.py` · 34 tests. Every figure is redrawn from `registry.tsv` by `figures/_regenerate_figures.py`, so the table is the single source and no number on these slides was typed by hand.
