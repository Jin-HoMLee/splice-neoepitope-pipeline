...
Of **96** registry rows, **92** pass both gates; **79 are scorable** - a published sequence + an HLA restriction + a positive readout.

::: {.callout-tip}
79 peptides is the entire field-wide positive base. We then ask: how concentrated is it?
:::
...
The effective number is the honest headline: **10 studies that are really worth ~3.7.**

## Result 1 - a few-study assembly

![](outputs/fig_study_pareto.png){width="78%"}

Top study (Bigot SF3B1-UM [@bigot2021sf3b1]) = **44%**; top two (+ Kim leukemia [@kim2025sfmutant]) = **67%**. Effective ≈ **3.7** independent studies.

## Result 2 - an HLA-A\*02:01 monoculture

![](outputs/fig_allele_mechanism.png){width="92%"}

**70/79 (89%) are A\*02:01** - effective ≈ **1.27 alleles**. Mechanism spread is healthier (effective ≈ 4.2). Any allele-stratified result collapses to an A2 result.

## Result 3 - the negative set is the binding constraint

![](outputs/fig_negative_scarcity.png){width="74%"}

**One** hard true-negative exists field-wide [@fisher2026corest]; 8 soft (failed-to-prime [@manoharan2026ircrc]). A powered AUC probe needs **19-31** negatives [@hanley1982roc]. Specificity is, today, **unmeasurable** (n = 1).
...
- **79 is a floor.** Sequence-blocked gate-passers (Zhao 2025, GNAS/RPL22) excluded pending retrieval; folding them doesn't relieve the A2 / negative constraints.
