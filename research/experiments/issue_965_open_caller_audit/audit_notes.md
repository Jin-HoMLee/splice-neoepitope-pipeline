...
- **Smoke:** #566's `run_asneo_chr22.sh` is a true end-to-end smoke but STAR-gated. The minimal ASNEO-only smoke is one command given an `SJ.out.tab`.
- **Verdict: GO-with-caveat** (open-only solved by #566; local run needs a pre-made junction table + the MHCflurry notebook). Recipe pointer: [`install/asneo_notes.md`](install/asneo_notes.md).
