...
- `outputs/shift_gap.png` — LOCO vs within-cohort validation gap: visualises the cross-lab/assay transfer drop (the "proxy caveat" gap). The within-cohort bars carry a variance band (mean +/- 1 SD over 25 CV-split seeds, [Issue #954](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/954)) so the gap is read against fold-partition noise, not a single draw. **Stability:** every cohort's shift gap is robust to the CV fold partition (its 95% partition band excludes 0), but only TESLA (+0.17) and HiTIDE (+0.11) are practically large; NCI (+0.02) and IMPROVE (+0.01) clear the partition noise only because 5-fold CV is stable across partitions for them (tiny per-seed variance), so their transfer penalty is real but negligible. The band captures fold-partition (Monte-Carlo) variance only: it excludes the finite-sample sampling error of the within/LOCO AUPRC and treats LOCO as fixed, so it speaks to robustness of the within>LOCO ordering, not statistical significance of the true gap.
...
- `outputs/auprc_explained.png` — teaching figure: **how AUPRC is built**, on real LOCO predictions. Left: ranking → threshold sweep → precision/recall (TESLA candidates). Right: the precision–recall curves whose **area = AUPRC**, with the prevalence baseline, contrasting TESLA (signal, ~5× lift) vs IMPROVE (near-random, ~1× lift). Numbers reproduce the LOCO/lift tables exactly.
...
|---|---|---|---|---|
| NCI | 0.000245 | 0.0359 | ~147× | 0.9999 |
| TESLA | 0.046 | 0.2482 | ~5.4× | 0.8779 |
| HiTIDE | 0.026 | 0.0733 | ~2.8× | 0.7816 |
| IMPROVE | 0.027 | 0.0322 | ~1.2× (near baseline) | 0.2678 |

*(Deterministic values from [`notebook.ipynb`](notebook.ipynb) - the source of truth - after the [#805](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/805) monotone-extrapolation change; refreshed in the notebook by [#1059](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1059) and mirrored here by [#1065](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1065). This table is a mirror; if the two ever disagree, the notebook wins.)*
...
NCI's headline "~147× lift" is largely a **tiny-prevalence artifact**: at a 1-in-4,000 positive rate the AUPRC/prevalence ratio explodes, so 147× is not 27× "better ranking" than TESLA's 5.4× - AUPRG places all three of NCI/TESLA/HiTIDE in the strong band (0.9999 / 0.878 / 0.782) and IMPROVE alone near random (0.268).
**Honest caveat on AUPRG at extreme imbalance:** AUPRG is prevalence-independent as a *population* quantity for a fixed ranker, but at NCI's 1-in-4,000 prevalence the precision-gain baseline is trivially easy to beat, so the *finite-sample estimate* saturates toward 1 - an estimation artifact, not a defect in the metric. NCI's post-#805 AUPRG of **0.9999** is that saturation in its clearest form: read the extreme-prevalence cohort with the same caution as its lift, and do not read the 4-nines as four significant figures of discriminative power.
AUPRG earns its keep most clearly on the three moderate-prevalence cohorts (~0.03-0.05), where lift compresses them into 1.2-5.4× while AUPRG cleanly separates the two strong discriminators (0.88 / 0.78) from near-random IMPROVE (0.27).
...
> **Update ([Issue #805](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/805), 2026-07-04): the out-of-support region is no longer a flat constant.**
> #805 replaced the `np.interp` endpoint-clipping with monotone linear extrapolation, so floor/ceil-mass scores now map to **ordered** log-odds instead of tying at the boundary value.
> The `out_of_calibration_support` flag and the floor/ceil **percentages are unchanged** (the support interval is the same), so the covariate-shift argument above holds verbatim.
> Only the *consequence* changes, and in the benign direction: where the text above says the shift lands "in the flat-extrapolation region where the log-odds is constant regardless," read it as the **ordered**-extrapolation region.
> The "benign for ranking" verdict is **strengthened**, not weakened: on the SNV subsample, per-cohort AUPRC under extrapolation equals the raw-score AUPRC exactly, whereas clipping had dragged it below (NCI 0.186 -> 0.153, TESLA 0.672 -> 0.651), because a strictly-monotone transform preserves ranking.
...
2. Rows whose `genotype_presentation_score` falls **outside** the interpolation support `[cx_[0], cx_[-1]] ≈ [0.0163, 0.9839]` carry an **`out_of_calibration_support`** flag — covering both the 21.4% floor mass **and** the 0.19% ceil mass beyond the knots. Post-[#805](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/805) these are **monotone-extrapolated (ordered)** rather than clipped to a flat constant, so the value is ranking-usable while the flag keeps it from being read as a fitted (calibration-accurate) value.
