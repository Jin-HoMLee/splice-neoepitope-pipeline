...
<!-- "Why now" callout removed — three r-fit-text lines fill the 720px slide height
already; the callout was cut off in the rendered HTML. The framing is restated
on the next slide ("Why this matters"). -->
...
%%| fig-width: 9.5
flowchart LR
    T["Tumor RNA-seq<br/>(SRR9143066)"] -->|"HISAT2 +<br/>regtools"| TJ["Tumor<br/>n = 1,872"]
    MN["Matched-normal<br/>(SRR9143065)"] -->|"HISAT2 +<br/>regtools"| MS["MN<br/>n = 1,714"]
    GT["Snaptron<br/>GTEx v2"] -->|"≥1 sample,<br/>chr22"| GS["GTEx<br/>n = 880,769"]
    AG["AlphaGenome<br/>predictions"] -->|"τ = F1-max"| AS["AG-normal<br/>n = 448"]
    TJ --> R{{"tumor ∩<br/>each filter"}}
    MS --> R
    GS --> R
    AS --> R
    R --> V[["Venn +<br/>decision"]]
...
<!-- Detail bullets ("AG does not add filtering value" + "filter stack stays MN+GTEx")
removed — two r-fit-text lines fill the 720px slide height; bullets overflowed
the bottom. The bullets restate the Decision rule slide (slide 8) findings,
so dropping them here is information-safe. -->
