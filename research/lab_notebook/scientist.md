...
**Recommended verdict (d) Hybrid, routed to Jin-Ho/PM:** stop committing *dedicated* functional-positive mining cycles (marginal yield ~0; #1207 = 0/3 catalogs), keep the passive Beat-1 watch (still feeds the MS tier #681), redirect dedicated effort to the negative bottleneck that gates the #585 calibration - positives do not.

**The deck's verdict shifted *because* Jin-Ho found a hole in it, which is the episode's real lesson.** My first draft recommended "(c) Hybrid: negatives need wet-lab/author-data." Jin-Ho asked whether we'd ever tried *reverse*-discovery - take every immunogenicity-DB epitope regardless of annotation, map the *sequence* back to the genome, and test junction-crossing (since splice origin is badly annotated in these DBs, which #734 had already confirmed row-level). We'd done the *annotation*-based mine (#734/#838, the "floor" - search splice-*titled* references) but never the *sequence-remapping* "ceiling." A feasibility probe against the IEDB IQ-API grounded it hard: 66,358 human-source T-cell assays, ~100% source-anchored, **32,684 NEGATIVE** - a ~33K-deep *computational* source for the negative class that is the deck's binding constraint (the registry holds 10 negatives, only 1 a gold-standard hard-negative). The re-review sharpened this further: an IEDB T-cell negative gives splice-derived + functional-negative but not the MS-presentation a *hard*-negative also needs, so remapped hits likely land candidate/soft-grade - they improve the 86:10 balance without necessarily adding hard-negatives. So the verdict shifted to "(d) Hybrid: reverse-map IEDB for negatives *first*, wet-lab only for the residual," filed as #1290. The methodological through-line is my own "a search key must not mirror the gate" rule: our free-text mine keyed on the authors' splice-awareness (the gate); the sequence remap removes that dependency. I overstated exhaustion in the first draft; the probe (verify before asserting, either direction) turned the hunch into a grounded option and corrected the deck before it shipped.
...
**I filed an Issue that was wrong, executed it correctly, and would have shipped a category error** ([Issue #1120](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1120), [PR #1186](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1186))

[Issue #1120](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1120) asked for an "animal model" value inside `assay_context`. I wrote it, PM triaged it, it read as an obvious S. Grounding it against the live registry before starting is the only reason it did not ship.

**`assay_context` is a T-cell-source axis** - every value names *where the responding T cells came from*. **An animal model is a setting** - where the experiment ran. Both rows the Issue targeted are **human T cells in a mouse arena**, and I verified both first-hand rather than off the readout string: Xiong's RCAN1-4 is an orthotopic xenograft in **immunodeficient NSG** mice with TCR-T transfer (read from the PDF), and COL6A3/Immatics is NSG plus **adoptively transferred human primary T cells** (PMC10130759). **An NSG mouse cannot produce T cells at all.** So recording either row's T-cell *source* as "animal" is not a judgment call I disagree with - it is false, and on COL6A3 it would have **overwritten `cloned_tcr`**, deleting the exact fact the column exists to hold. With zero animal-only rows, the value I asked for would have had **no correct members**.

**The shape of this failure is the one I keep meeting.** Nothing about the Issue was careless. The defect was invisible from inside it, because the Issue was reasoning about *whether an animal readout deserves to be distinguished* (yes) and never about *which axis the distinction lives on*. Every check I own operates on the answer; nothing operates on the question. Yesterday's lesson was to turn the falsifier on my own apparatus - **today it was the Issue itself that needed falsifying, and an Issue is not usually a thing I think of as a claim.** It is: a filed Issue asserts that its premise holds, and it is entitled to be wrong.

I reshaped the ACs **on the Issue, before building**, and struck the originals in place rather than deleting them. That is the discipline I got caught on yesterday and I wanted the trace to exist: an AC quietly rewritten to match what shipped can no longer fail.

**Then the bot found the same class of error inside my fix, twice.**

**One: I shipped a falsifier that could not fail.** `derive_in_vivo_model.py` compared the *just-derived* column against the readout marker and called it a cross-check - but `df.apply()` had already overwritten any hand-edit, so it was **the derivation checked against itself**, structurally green whatever the file said, with a comment asserting it caught hand-edits. I have a standing rule that a check which cannot fail is a ritual that returns green. **I wrote one anyway, in a PR whose entire pitch is falsifiable guards.** Repaired to read the on-disk column *before* deriving; matched-pair verified (exit 1 on a planted edit, exit 0 clean).

**Two, and this is the one worth keeping: the reviewer proposed a coupling that was *correct given my definitions*, and the definitions were the bug.** It argued `in_vivo_model == syngeneic` implies `assay_context == animal_syngeneic`, quoting my own constants back at me: `syngeneic` = *"immunocompetent host responding with its OWN T cells."* Under that wording the inference is valid. But **that wording is a T-cell-source claim living in the setting vocabulary** - the exact conflation the PR exists to remove, re-created *inside the fix*, by me, while writing the docstring explaining why the conflation is bad. Once the setting values describe the **host and only the host**, `syngeneic` + `cloned_tcr` becomes representable - an immunocompetent host receiving adoptively-transferred cloned *mouse* TCR-T cells, a real design my vocabulary had made unsayable. So the converse coupling would have **forbidden a legitimate experiment**, and I rejected it. The forward direction (`animal_syngeneic` ⟹ `syngeneic`) stands: a source constrains the host; a host does not constrain the source.

The reviewer noticed the counterexample and **talked itself out of it** - *"but that arguably contradicts the 'its OWN T cells' definition anyway."* It contradicts the definition. It does not contradict reality. **When an experiment you know to be real is unrepresentable in your schema, the schema is the thing that is wrong**, and a definition is not evidence.

**Lesson.** Three separate times today the error was one level up from where I was looking: in the Issue's premise, not its execution; in the check's construction, not its result; in the vocabulary's wording, not its application. Each time the layer below was *impeccable* - correct code, passing tests, a validator that went green throughout. **Green tests are exactly what a wrong frame produces, because the tests are written in the frame.** The only thing that has reliably caught these is a collision with something outside the frame: a real row, a real mouse strain, a real experiment that would not fit. **Keep touching the artifact.** The registry, the PDF, the actual NSG mouse that cannot make a T cell - those are what disagree with me. My reasoning never does.

### 21:39 UTC - Editor: Scientist

**A filter that would have deleted the thing we are hunting - and its validation could not have failed** ([Issue #1116](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1116), [Issue #699](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/699), [PR #1182](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1182))

Developer handed me the threshold decision on the junction repeat-embedding score: a `min_hamming > 2` filter keeps 99.3% of GENCODE-annotated junctions and deletes 88.5% of production candidates, and he correctly refused to decide which of those two facts was the headline. **Verdict: do not gate, at any threshold.** Three independent reasons, and only the first was on the table when he asked.

**The falsifier was confounded.** He validated the Hamming score against RepeatMasker overlap, calling it a completely different method. His marginals looked twinned (annotated n=287 / unannotated n=1,585 against not-in-repeat n=299 / in-repeat n=1,573), so I cross-tabulated rather than reading the margins: the two partitions are **98.4% concordant, phi = 0.940**. On this fixture "in a repeat" is nearly a synonym for "unannotated". Once the score separates annotated from unannotated at 120x, the RepeatMasker agreement is **entailed, not earned** - Q1 wearing a hat. Stratify to hold annotation fixed and it collapses: the 9 annotated junctions that RepeatMasker places *inside* repeats are the cleanest test available (repeat-embedded *and* real), and the score calls **0 of 9** suspicious at a median distance of 6.0. Where the two methods are free to disagree, they do.

**It fails the exact selectivity test he used to kill the NH gate.** He condemned that gate because it removes 85% repeat-embedded junctions while keeping 70% - "not selective for this signal". Turn it on his own score: survivors of `min_hamming > 2` are **96.3% repeat-embedded**, against 98.7% before filtering. It deletes 85% of the pool and leaves the survivors exactly as repeat-y as it found them. He held the NH gate to a standard he never applied to his own instrument, which is not a lapse so much as the default condition - **your controls interrogate the thing you are testing, never the thing you are testing it with.**

**The biology prior is refuted, not absent - and that is the part that decides it.** His route 3 asked whether a real neojunction is a priori unlikely to sit in a repeat, and he said he had no basis to hold that belief. He was right not to, and the literature says the belief runs backwards. The candidate pool is **95% SINE** (1,491 of 1,564) - in human, Alu. Alus harbour cryptic splice sites; an antisense intronic Alu is a documented template for new exon creation; ~5% of human internal alternative exons are Alu-derived. And exon-TE splice junctions are already a **validated immunogenic recurrent neoantigen class** - the two *Science Immunology* 2023 companions (`5ZADNVCB`, `29W9RRV7`), both of which are on our own shelf and are the papers [Issue #699](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/699) was filed on. So "unannotated junction anchored in a SINE" is **simultaneously** the signature of alignment noise and of the antigen class this pipeline exists to find. The score cannot separate them; neither can I. That is precisely why it must not be a gate - **a gate resolves the ambiguity silently, and in the direction that deletes the target.**

**The near-miss worth naming.** A `min_hamming > 2` merge would have answered [Issue #699](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/699) in the negative, by construction, before anyone decided it. #699's own body anticipates the hazard from the *aligner* side ("silently dropped if the aligner doesn't report junctions anchored in repeat-masked/TE regions"); this was the same hazard arriving from the *filtering* side. Two views of one decision, and the scope call has to come first. Constraint recorded on [Issue #1038](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1038) too: the simulated validation set is the only route that could ever *earn* a threshold, and it must inject true junctions **into** Alu/TE contexts - a simulation that injects them only into unique sequence makes "repeat-embedded implies false" true by construction and would hand back a triumphant, worthless threshold.

**And the cost lands on the best candidates, not the junk.** I expected the score to mostly prune the 1-read noise (90.2% of the pool). It does not: unannotated junctions with >= 2 reads (n=155) have a median `min_hamming` of 1.0, and only **16 of 155 survive** the filter. It deletes 90% of the depth-supported candidates. I am not claiming those 139 are real - multimapper pile-up in an Alu manufactures 2 reads easily - only that the damage is concentrated where the remaining signal is.

**Quick-win burn-down, and one finding inside it.** Two PRs to the gate. [PR #1182](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1182) canonicalizes the registry's `source` key (15 strings for 12 studies). The interesting part is *why nothing caught it*: every consumer resolves `source` through a **lowercased-substring** map, so both spellings of a study matched the same key and derived identically - **the split was invisible precisely because the derivations were correct.** It was not harmless (the [#737](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/737) sparsity notebook was double-counting Xiong). The new guard is a **registry-level** invariant - it cannot be expressed by looking at any single row, which is exactly why a validator otherwise dense with per-row checks never saw it. [PR #1183](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1183) refreshes the calibration README's stale LOCO table, whose figure caption asserted "~4x lift" one line after claiming "numbers reproduce the tables exactly".

**Lesson.** Yesterday's entry ended on the counter-move of building the reference's own background before believing a table. Today the same move worked on someone else's apparatus: the cross-tab that killed the RepeatMasker validation took two minutes and existed only because I distrusted a pair of suspiciously twinned marginals. **What I want to keep is that the score is not *wrong* - it measures something real, and Developer's work on it was careful and honest. It is unusable because the thing it measures is confounded with the thing we want.** That is a failure of the *evaluation frame*, not of the instrument, and no amount of rigor at the point of measurement would have surfaced it. The question that did: *what would this filter's excellent performance look like if the population it excelled on were the one population it structurally could not hurt?*

### 16:38 UTC - Editor: Scientist

**Both AC-4 and AC-7 came back negative, and the negatives are what promote AC-10** ([Issue #681](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/681), [PR #1172](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1172))

Delivered the two remaining tractable ACs of the ligandome reanalysis. The headline is that neither produced the result it was reaching for, and that is the finding rather than a consolation.

**AC-4 - the presentation tier exists, but only against one reference, and only above a real background.** 29 peptides are both predicted splice neoantigens and observed in SysteMHC's non-UniProt set (4 MS-confirmed by SNAF). They land as `label=untested` / `tier=presentation-prevalence`: presentation evidence, never functional positives. I nulled them, because an exact 8-11mer match between an 87k panel and an 8.9k atlas is not self-evidently signal - short peptides, 20-letter alphabet, composition-biased at anchors. A composition-preserving shuffle recovers **0.010** hits on average; the conventional reverse decoy recovers **3**. So the honest statement is *~29 against a background of ~3*, not against zero. **Our own panel contributes 0 of the 29 at an expectation of 0.72** - and I labelled that underpowered rather than specific, which is the same call I got wrong on 2026-07-13 and had to retract publicly.

**AC-7 - the attractive answer was sitting right there, and it is wrong.** The recovered set spans 25 alleles with A\*02:01 at 3.6%, which reads as the classical A\*02:01 dominance of immunopeptidomics being broken by splice peptides. It is not. The **reference's own** A\*02:01 share is 6.3%, so we expect 1.77 and observe 1: no depletion. SysteMHC leans on mono-allelic cell lines and is allele-diverse *by construction* (103 alleles), most of its rows carry no allele at all, and 11 of our 29 peptides carry none. The reviewer sharpened the argument better than I had: **28 assignments over a 103-allele reference means nearly every cell is expected to be 0 or 1, so a "1 each" table is exactly what sampling noise looks like.** A per-allele table here restates the reference's composition instead of measuring ours - the search-key-mirrors-the-gate shape, for the third time this week, in a costume I did not recognize until I built the background control.

**Why the two negatives converge.** AC-2 had already measured that a canonical-search atlas has a structural ceiling of ~zero for junction-spanning peptides. AC-7 now measures that the one non-canonical atlas we can borrow reports through a column we do not control. Both terminate in the same sentence: **you need your own search.** AC-10 (re-search raw spectra against a junction-derived non-canonical database) is promoted from stretch goal to the deliverable. Borrowed-atlas intersection has been taken as far as it goes.

**The bot review found a real defect, and the evidence was already in my own repo.** `decoy_null` iterated a **set of strings**. Python hash-randomizes `str` per process, so the RNG stream was consumed in a run-dependent order: **the seed pinned the generator but not the order it was drawn from.** The consequence was a drifting decoy mean - and I had committed a README saying `0.01` alongside a JSON saying `0.02`. Two artifacts in one PR disagreeing with each other, and I shipped it. I have a rule that an unexplained delta is a stop condition, and I walked past a two-file contradiction of my own making. Fixed with `sorted(panel)`, then verified the way it should have been verified the first time: two runs under different `PYTHONHASHSEED` values now produce byte-identical output. A matched-pair control, not an inspection.

**A provenance gap in the same family.** The committed `caatlas_peptides.txt.gz` is the AC-2 ceiling-control input - half the headline finding rests on it - and its only regenerator was my shell history. Committed the distill step, and checked it against the falsifier: re-distilling the raw harvest reproduces the committed 195,217-peptide set exactly. Had it not, the chain I was documenting would have been fiction. Related: the caAtlas gene list is HGNC **protein-coding + Approved** (19,273), not the complete set (45,021) - a naive re-fetch would have harvested a different gene space, which I only noticed because I checked the number against the live source instead of trusting the filename.

**Infrastructure finding, filed as [Issue #1173](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1173).** The PR's auto bot-review never fired. Hooks are resolved from the **working tree**, and this branch was cut before the [#1142](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1142) newline fix landed - so it was still running the old, blind hook. **A hook fix protects only the branches cut after it**, every in-flight branch silently keeps the bug, and the guards' own fail-open discipline guarantees nobody notices. That applies to every rung-3 mechanism we have shipped, not just this one.

**Lesson.** Three times this week the wrong answer was the *attractive* one, and each time what saved me was a control I built before I knew I needed it - the ceiling check, the composition-matched decoy, the allele background. **The pattern is not that I keep making a new mistake; it is that I keep making the same one wearing different clothes: accepting a number whose only virtue is that it points the way I expected.** The counter-move is boring and reliable - before believing any table, ask what the *reference* would produce on its own, and refuse to publish the number until you have that in hand.
...
#### [PR #1093](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1093) - pVACtools runs MHCflurry-only, class I, zero NetMHC. Closes [Issue #1048](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1048).

**Context.** The linchpin feasibility spike under the comprehensive-toolkit thesis ([#1045](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1045)): *adopt pVACtools' source breadth, run it on our open MHCflurry stack.* pVACtools is BSD-3-Clause-Clear and therefore reusable, but its published pipelines route through NetMHCpan/NetMHCIIpan, which are academic-license-only and blocked for us. If pVACtools cannot be driven on MHCflurry alone, Layer A of the architecture is a partial rebuild rather than a wrapper.

**Verdict: GREEN.** pVACtools 7.0.1 runs `pvacseq` (VEP-annotated VCF, 9,132 epitopes scored, 27 filtered) and `pvacbind` (peptide FASTA, 37 scored, 3 filtered) end-to-end for class I on MHCflurry alone, with no NetMHC binary, no IEDB standalone, no `mhcnuggets`, and no `tensorflow` anywhere in the environment. Layer A is a wrapper job. Write-up + reproduce script: [`research/experiments/issue_1048_pvactools_mhcflurry_spike/`](../experiments/issue_1048_pvactools_mhcflurry_spike/README.md).

**The finding that actually matters, and it is a licensing one.** `pvactools==7.0.1` declares **`mhcnuggets==2.4.1` as a hard install dependency** (not an extra), and mhcnuggets hard-requires `tensorflow`. mhcnuggets is **JHU non-commercial**. So *a plain `pip install pvactools` cannot produce an open-only environment* - it silently pulls a non-commercial package into whatever image we build. It is cleanly avoidable, because mhcnuggets is only *needed* when `MHCnuggetsI`/`MHCnuggetsII` are selected as algorithms: installing `--no-deps` and supplying the dependency list minus mhcnuggets leaves `MHCflurry`/`MHCflurryEL` fully selectable. But it must be done deliberately. That is now an explicit constraint on the Layer-A env spec, recorded on #1045.

**Why it works at all: subprocess coupling, not API coupling.** pVACtools invokes MHCflurry by shelling out to `mhcflurry-predict --alleles ... --peptides ... --out ...`, never importing it as a library (`pvactools/lib/prediction_class.py:516`). Three consequences. (1) Its `mhcflurry==2.0.6` pin can be overridden: 2.0.6 lazily `import tensorflow` inside `Class1NeuralNetwork.merge()` at *predict* time - an undeclared runtime dep, so it installs fine and crashes on use - whereas our production 2.2.x on torch works. (2) **Our MHCflurry version is the one that scores**, which is precisely the separation of concerns the 3-layer architecture assumed but had never verified. (3) The CLI contract, not the Python API, is the integration surface, so I bounded the pin at `<3` rather than leaving it open.

**Evidence, not exit codes.** The `pvacbind` fixture seeds three known HLA-A\*02:01 epitopes inside longer sequences; of the 37 scored 9-mers, exactly `GILGFVFTL` (influenza M1), `SLYNTVATL` (HIV gag) and `KLQEEIPVL` survive filtering. The count itself falls out of the fixture lengths (13+12+12=37), so it is internally checkable. `reproduce.sh` was executed end-to-end from a clean venv and both committed outputs reproduce byte-identically.

**Six environment gotchas, all encoded as guards or comments in `reproduce.sh`.** The mhcnuggets/tensorflow exclusion; the mhcflurry pin; a **bare-name `PATH` lookup** whose failure surfaces as `An error occurred while calling MHCflurry:` with an *empty* message (a swallowed `FileNotFoundError` - calling `pvacbind` by absolute path is not enough); undeclared `h5py` (normally arrives via the tensorflow stack); `setuptools<81` because mhcflurry still imports `pkg_resources`; and `tkinter` required at package import (`lib/vector_visualization.py` imports `turtle`), which rules out pyenv 3.11.5 and the homebrew builds here. Separately, a **local toolchain defect, not a pVACtools problem**: this machine's `clang++` cannot compile *any* C++ because the CommandLineTools `c++/v1` directory lacks the libc++ headers, breaking the `vaxrank` -> `shellinford` build; worked around with `-isysroot`/`-I` at the SDK copy.

**Bot review: LGTM**, with four non-blocking notes, all addressed in `19f72c1`. Two were genuinely worth taking: the `mhcflurry>=2.1` pin was unbounded upward on a load-bearing version, and the committed-vs-fresh diff was **advisory only**, so a zero exit did not actually enforce the byte-identical claim the README made. Added an opt-in `STRICT=1` that makes divergence fatal (default stays advisory - float jitter across torch builds is expected, not a defect), re-ran under `STRICT=1` to substantiate the claim, and made the README state plainly what a plain exit 0 does and does not mean. I also out-verified the review on one point: it suggested production peptide lengths span 8-11, but `config/config.yaml:98` sets `[8, 9, 10]`, so the wrapper note records the verified range.

**What this does NOT settle, and I want it on the record rather than buried.** `pvacsplice` - the module closest to this project's core - was **not run**. It needs a regtools junction file plus reference FASTA/GTF, and its published path is mutation-driven (a DNA VCF alongside the junctions). Whether it can be fed junction-only is the Issue's secondary risk, and it remains open; that is the natural follow-up spike. **Class II is unchanged and still blocked**: every class-II algorithm pVACtools offers is NetMHC-family or non-commercial (`NetMHCIIpan`, `MHCnuggetsII`, `MixMHC2pred`, `NNalign`, `SMMalign`), so absorbing pVACtools buys **zero** class-II coverage. That is consistent with [#1049](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1049), and it means the differentiation thesis still rests entirely on the class-II frontier, which this GREEN does not advance. Only macOS arm64 was exercised; the tkinter and shellinford gotchas are macOS-flavored and a Linux/CI env will hit different ones.
...
The stored LOCO/AUPRG tables + 7 figures predated the calibrator scoring-path changes in [#805](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/805) (monotone extrapolation instead of endpoint-clipping), [#804](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/804) (Jeffreys CIs), [#803](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/803) (AUPRG) and were never re-run against them.
Split out from #954 (Jin-Ho's call) so the headline-number refresh is reviewable in isolation from the variance-band methodology change.

**What shipped.** Re-execute end-to-end (no validation-logic change) + correct the hardcoded numbers/verdicts in prose that cited the pre-#805 values.
LOCO AUPRC moved: NCI 0.0271->0.0359, TESLA 0.2013->0.2482, HiTIDE 0.0706->0.0733, IMPROVE unchanged; `cox_slope`/`top20`/`prior` unchanged (the extrapolation reorders tail points, which AUPRC/AUPRG are sensitive to).
A notable second-order effect: after #805 the adaptive-vs-fixed KDE gap collapsed to **zero on every cohort** (was a TESLA-only -0.012), so the "only material difference was on TESLA" verdict in cells 21/34 became false and was rewritten.

**Verified.** Standalone recompute reproduces the refreshed numbers exactly (deterministic, NCI 0.0359 / TESLA 0.2482); source diff confined to cells 15/21/31/34/40 (markdown + one code print/comment); 8 data-driven figures regenerated byte-identically across two executions; no stale pre-#805 token remains in any source cell (grep-verified).

**Bot review (`10m 6s`).** LGTM on the mechanical refresh (verified byte-level "no source change" on the original outputs-only commit, cross-table internal consistency, deterministic re-fit). One correct non-blocking flag: the stale-prose deferral was under-scoped - the same numbers lived in cells 15 (a data table), 34, 40, and a hardcoded print in cell 21's *code*. Addressed in `1b69b14` (verified each claim first) rather than deferred; only the within-cohort interpretive reframe rides #954.
Held at the merge gate for Jin-Ho's final look.
...
**What.** [#940](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/940) made a blood-vs-solid normal exclusion explicit at the *data* layer (a `normal_source` column on the junctions TSV + `normal_shared:<type>` breakdown rows in the junction-filter stats), but `generate_report.py` dropped the per-source rows via the `funnel_cats` `.isin()` filter, so a scientist reading `report.html` still saw only an undifferentiated `normal_shared` total. New pure helper `_normal_shared_by_source()` pivots the `normal_shared:<type>` stats rows into a per-sample breakdown table, appended to the filtering-funnel section when present (omitted for pre-#940 stats). So the blood-only / weak-normal case (patient_002's CD3+ PBMC normal vs an osteosarcoma) is now visible at a glance - the "explicit, not silent" goal of #940 lands in the human-facing surface.

**Design choice.** Used the stats-TSV path (the `normal_shared:<type>` rows #940 emits for exactly this) over re-deriving from the junctions-TSV `normal_source` column. More aligned with #940, and it sidesteps the NaN round-trip the #980 review's obs #3 warned about: on this path the categories are pre-aggregated strings, the reader already `.fillna("")`s, and `filter_junctions.py` only emits a `normal_shared:<type>` row for a type that removed >=1 junction (real integer count, no NaN). Counts cast to `int` so they render `154` not `154.0`; an empty type suffix renders `(unspecified)`.

**TDD.** Tests written first (pure-helper pivot + reindex-fill, empty-frame back-compat, render surfaces / omits), watched fail (`AssertionError` on the missing render section + `ImportError` on the missing helper), then implemented to green. Rendered the actual HTML for a blood-only patient_002-like case to eyeball it (clean: no `NaN`, counts as ints, blood-source label present).

**Review.** Bot review LGTM, no blocking issues - it traced the helper against the #940 emitter and confirmed the dtype/NaN reasoning. Addressed both non-blocking nits: added a test for the `(unspecified)` empty-suffix branch (was defensive-but-untested) and dropped two redundant local test imports. Suite 131 report+stats tests green.

#### [PR #1015](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1015) - `venue_type` provenance column + validator guards + offline-optional Zotero cross-check on the splice-immunogenicity registry. Closes [Issue #1001](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1001).

**What.** The registry had no structured field separating peer-reviewed journal sources from preprints (venue lived only in free-text `source` / PROVENANCE prose), so it was not queryable, enforceable, or auditable - standard evidence-grading (GRADE) down-weights non-peer-reviewed sources. Added a `venue_type` column (`journal | preprint | db_recovered`) with the source map in `labeling_constants.py` (shared source of truth, so derivation + validator can't drift), a source-keyed `derive_venue_type.py` mirroring the sibling `derive_assay_context.py`, and validator enforcement. All 11 studies (14 `source` strings, 96 rows) back-populated `journal` per a first-hand 2026-07-04 venue audit; 0 preprints (the 3 preprints encountered were each deferred for sequence-unavailability, not venue).

**Design decision - Zotero as verifier, not derivation source.** The question of whether to derive venue live from Zotero's `itemType` came up. Zotero is the authoritative journal-vs-preprint signal (it caught my errors, below), so it is the right *input to the audit* - but a live dependency was rejected: it would couple a committed column to a network service and break hermetic CI / offline / cloud-sandbox runs (no Zotero creds there), some rows are `NOT in Zotero`, and a committed artifact shouldn't need a mutable external service to regenerate. So the stored value stays a repo-local map and the validator carries an **offline-optional cross-check** (the `annotate-canary` pattern): when `ZOTERO_*` creds are present it resolves each source by DOI in `Z38GTJNW` and fails on any `venue_type`<->`itemType` disagreement; with no creds it skips cleanly. DOI-keyed, so it sidesteps the parent-vs-attachment-key trap.

**Cross-check earned its keep immediately.** Running it caught hand-entry errors in my own venue-name audit table (Bigot = Cancer Discovery, not JCI Insight - which is actually Fisher; Xiong GBM = Cellular & Molecular Immunology; Manoharan = Scientific Reports). `venue_type` values were unaffected (all `journal`, independently confirmed by Zotero `itemType`: every source resolves to `journalArticle`). Verified against Zotero `publicationTitle` and fixed before merge.

**Review.** Bot review non-blocking. The sharp finding (#1): the source-substring key can't self-distinguish a preprint from the journal version of an *already-mapped* study (a future `SNAF ... bioRxiv` source first-matches `snaf` -> `journal`), and - correcting the review's read - the Zotero cross-check does NOT backstop this, because it validates each *map entry's* DOI, not each row's actual source. Closed it structurally: `PREPRINT_MARKERS` + a validator check failing any row whose `source` names a preprint server but whose `venue_type` isn't `preprint` (verified it flags a bioRxiv-source-marked-`journal` row, passes a real `preprint` row, no false positives on journals). Also fixed the `.env` walk (`continue` past a creds-less `.env`), a louder WARNING on rejected-creds `HTTPError` vs quiet offline skip, the 11-vs-14 count wording, and a shadowing `v`.

**Verification.** `derive_venue_type.py` + `validate_registry.py` green end-to-end (exit 0); live Zotero cross-check passes (all 11 sources agree); skip path returns green with no creds; both new guards (unmapped-source sentinel, preprint-marker) confirmed to fire on synthetic bad input and pass clean input. `registry.tsv` diff is column-only (97/97 lines, zero row-content mutation).
...
#### [PR #961](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/961) - Jeffreys/boundary CI for the reliability plot instead of Wilson. Closes [Issue #804](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/804).

**What.** Replaced the Wilson score interval on the #547 immunogenicity-calibrator reliability plot with a Jeffreys (boundary-corrected) interval. New `reliability.py` with a tested `jeffreys_ci(n_pos_w, n_total_w, n_eff, alpha=0.05)` helper - the equal-tailed Beta(1/2, 1/2) credible interval (Brown, Cai & DasGupta, *Statist. Sci.* 2001) on the Kish effective-n scale, with the boundary rule (lower=0 at zero effective successes, upper=1 at all). Repointed the notebook's `reliability_curve` from `wilson_ci` to `jeffreys_ci`, removed the extracted `wilson_ci` def, updated the "Wilson CIs" prose in the notebook + README, and regenerated `outputs/pr_reliability.png`.

**Why (the purpose, since the CI is diagnostic-only).** The reliability-plot error bars are a *visual diagnostic* (per #592), not load-bearing for any decision. But Wilson under-covers as the observed rate p -> 0, and our lowest-prevalence held-out cohort (NCI, prevalence ~ 2.45e-4) sits exactly in that boundary regime, so the whiskers there were systematically too narrow. Jeffreys reaches coverage close to nominal at the boundary, so the plotted uncertainty is honest where it matters most for us.

**The finding (quantified the premise, did not assume it).** A seeded Monte-Carlo coverage check at our regime (p=0.002, n=500, 20k draws) put Jeffreys at ~0.98 empirical coverage vs Wilson's ~0.92 (nominal 0.95) - confirming Wilson's boundary under-coverage first-hand rather than citing it. Jeffreys is mildly conservative (over-covers) at extreme-low p, which is the right failure direction for a diagnostic.

**Rigor.** Followed the experiment's `prg_metric.py` extract-and-test convention: pinned the interval against `scipy.stats.beta` known-answer literals, plus boundary behavior, effective-n width control, degenerate-input, and the coverage check. The figure was regenerated by exec-ing the *real* notebook cells (3 -> 11 -> 13 -> 27) in one namespace (no plotting-code duplication), which reproduced the LOCO metrics identically - a de-facto end-to-end run. Diff scoped to 4 source cells + 1 figure preview; the other 10 embedded figures untouched.

**Review + fixes.** `@claude` review verdict: **LGTM**, all notes non-blocking. Hand-traced the Jeffreys formula, the float-safe boundary comparisons (`p` clamp makes `x_eff == n_eff` exact), the effective-n plumbing, and the boundary values flowing finitely through `_empirical_logit` - all confirmed correct. Applied the two clarifying suggestions in 80a308b (a docstring note that the `p` clamp is float-noise armor since reweighting keeps weights positive; a comment marking `wilson_cov < 0.95` as a seed-dependent *premise* guard, distinct from the load-bearing `jeff_cov >= 0.95` / `jeff_cov > wilson_cov`). Declined the third (range-validate `alpha`) - the reviewer also recommended against it for a one-caller internal helper.

**Verification.** 49 tests pass (13 new + 36 existing) in `research/.venv`; all 4 CI checks green (pipeline-pytest, ci-tools-pytest, snakemake-dry-run, conda-env-solve); coverage numbers read first-hand from the seeded simulation; regenerated figure visually verified (Jeffreys whiskers, 45 deg diagonal + Cox fit intact, no overflow). No pipeline runtime path touched - the calibrator's deployment output is unchanged.
...
> **Correction (2026-07-02, added during [Issue #636](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/636)):** the sentence above - "R2 holds only its pre-#370 vintage, so [patient_002's] prose must not be touched until its data is re-run" - is **factually wrong** and was written without checking R2. A **complete 2026-06-23 STAR run for patient_002 is on R2** (alignment, junction filter, peptides, `mhc_presentation.tsv`, TCRdock; `tumor_exclusive=42`), and the STAR path is inherently unaffected by #370 (a HISAT2/regtools coordinate bug); [#212](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/212) and [#378](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/378) are both **closed**. So patient_002 is **not** data-blocked. The *conclusion* (hold patient_002's manuscript prose) still stands, but for an entirely different reason surfaced by #636's AC3 comparability gate: patient_002's only matched normal is a **CD3+ T-cell PBMC "Blood Derived Normal"** (not tissue-matched to a bone tumor), so its `tumor_exclusive` set is unreliable *by construction* and must not be presented side-by-side with patient_001's. Lesson reinforced: verify as-shipped / data-availability claims against the bytes before writing them ([[feedback_verify_location_facts_against_source]]).
...
- **AC#4 — writeup:** `issue_734_db_audit/FINDINGS.md` (citable null + method) + `recovered_candidates.tsv` (55 candidates with provenance).
...
> **Update (2026-07-02, added during [Issue #636](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/636)):** "identical in kind to AC 7" above refers to the **filter-stacking code path** (matched-normal then GTEx applied the same way for both patients) - **not** to the biological adequacy of the two normals. #636's AC3 comparability gate reassessed the latter: patient_002's matched normal is a **CD3+ T-cell PBMC**, not tissue-matched to a bone tumor, so it cannot subtract bone/mesenchymal splicing and its `tumor_exclusive` set is **not** tissue-specificity-controlled. The mechanical stacking validation here stands; the *biological* comparability to patient_001 does not, so the manuscript now presents patient_002 as a matched-normal limitation case rather than a validated second result.
