...
**Made the closed-work recap cover both repos ([#1276](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1276), [PR #1302](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1302)) - and the work was not the loop over two repos, it was what became ambiguous once there were two.** `check_closed_recent.py` hard-coded the project repo, so the whole Memory Manager workstream was invisible to the mechanized recap; a two-day column of MM closes was silently omitted on 07-22 until Jin-Ho asked why MM work never appeared. A recap that cannot see a workstream reports "nothing closed" for it, which is the same silent-wrong-answer class the script was written ([#784](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/784)) to prevent, arriving through a different door.

**The real hazard: the two repos have colliding issue numbers.** Both have a `#98`, unrelated Issues. Spanning both repos without disambiguating would have traded a *missing-data* bug for a *wrong-attribution* bug - and the second is worse, because it looks correct. `repo_ref()` mirrors the `pers#` prefix `board_open_items.py` already uses rather than inventing a second convention. **The generalisable move: when adding a second data source, ask what becomes ambiguous, not just what becomes reachable.** I had hit the same collision by hand ten minutes earlier during Replenishment, reading the project repo's unrelated `#98` while the board meant the personas one.

**An AC I had not read caught a gap my own test design missed.** [#1276](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1276) had a fourth criterion - two-repo aggregation *with the `>=` boundary preserved* - and it was genuinely unmet: aggregation and the boundary were each tested, never together, so the boundary was untested in exactly the path this Issue added. Red-path probed the fix (flipping `>=` to `>` reds it alongside the two existing boundary tests), so it discriminates rather than passing by construction.

**Two review nits worth recording, for opposite reasons.** The bot found my `REPO` compat alias was justified by a comment claiming callers referenced it - a repo-wide grep says **nothing does**, so I had written a justification without checking the premise; removed rather than reworded, since dead code kept alive by an untrue justification is worse than none. And I fixed the duplicate-`--repo` double-count *against* the reviewer's "not worth it": a doubled row set in a recap is a silently wrong answer, the script's own signature failure, so one line of idempotence is cheap insurance.

**Pattern about my own output, second instance today.** The remaining nit was that my PR body described a refactor I had not done ("sorting moves out of `collect()`"). Combined with the wrong entry count on [PR #1297](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1297), that is **two description-vs-reality slips in one session, both in prose *about* verified code rather than in the code**. The code was checked; the sentences about it were not. A PR body is exactly what a reviewer trusts when they cannot run the tests - and this reviewer explicitly could not.

### 13:30 UTC - Editor: PM

**Closed [#250](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/250) (PM decision telemetry) as delivered - it was a *stranded card*, not unfinished work, and the two states look identical on a board.** It sat `In progress` for days with no branch and no PR, which reads as work-in-flight. It was not: `pm/pm_decisions.md` has been live since 07-20 with 6 logged decisions, `pm/feedback_pm_telemetry.md` documents the schema, and the weekly calibration review is wired into the morning routine. Every remaining AC was already satisfied; the two genuinely-open ones had been carried to [#1247](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1247) (the probation review, data-gated to ~2026-08-03) some time ago.

**The generalisable bit: a card whose deliverable landed in the *other* repo has no local artifact to make its doneness visible.** The work was committed to the personas repo by MM, so from the project repo there is no branch, no PR, no diff - nothing that says "this shipped". `In progress` then persists by default, and the board reports capacity that is not actually in use. This is the same cross-repo close-routing gap [#1260](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1260) exists to fix, showing up on the *status* axis rather than the *close* axis.

**Verified before ticking, and found one honest variance.** AC-2 specified appending decisions at the morning warm-up "and a log review on **Mondays**"; the implementation instead rides the **Friday-cleanup** beat, deliberately, on the documented rationale of "one weekly cadence, not two". I ticked it as satisfied rather than silently conforming the record to the wording, because the intent (a weekly calibration scan exists and is wired to a beat that actually runs) is met and the deviation is the better design. Ticking to the *intent* while naming the variance is the honest form of [[feedback-ac-gates-intent-not-proxy]]; quietly ticking to the *letter* would have hidden a real design choice.

**Method note for the next stale-card sweep:** `In progress` with **no branch and no PR** is a high-signal tell worth scanning for directly. It means one of three things - the card is stranded (this case), the work lives somewhere the repo cannot see (also this case), or it genuinely never started. All three want attention, and none of them is "work in flight".

### 12:50 UTC - Editor: PM

**Built the body-gated Ready guard ([#1248](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1248), [PR #1292](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1292)), and running it against the live board immediately caught a false positive in my own implementation.** The floor guard counted Ready *depth*, not *pullable* depth, so a prose-gated Issue read as healthy stock and got re-committed under floor pressure ([#841](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/841) swept in twice). `scripts/pm/not_pullable.py` is a pure function of the body with two tells, because the fixtures fail differently: #841's gate lives in `## Notes` (so an AC-scoped scan misses it entirely) while #929's is decision verbs inside ACs.
The [#1248](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1248) body offered "exclude from the count **OR** surface a `[NOT-PULLABLE]` line" and left the semantic to PM. It is **both**: exclude-only makes a gated item vanish silently, surface-only leaves the queue still reading healthy - which *is* the original bug. Neither branch alone fixes the stated problem, so an either/or framing in an AC is worth re-reading as a possible false dichotomy.

**The unit fixtures were green and wrong, and only real bodies showed it.** My hand-written #1248 fixture assumed the markers would be backticked. The live body instead *discusses* gates mid-sentence ("chartered to build only on a 2nd trigger event"), so the Issue introducing the scan tripped its own detector - the same "using a directive vs talking about it" hazard [#1126](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1126) drew for `skip-lab-notebook`. The fix was anchoring markers to a clause start, on the principle that **a gate is a directive and a directive opens a clause**. Generalises past the live-integration-smoke rule: a fixture I authored encodes my *assumption* about the input, so it can only ever confirm the shape I already imagined. Ten real bodies cost one command and falsified it.

**Precision-first was the load-bearing decision, and it came from our own rule.** By `feedback_fail_safe_not_fail_open`, this is a **linter**, not a gate, so a false ALARM is the dangerous direction (an over-eager guard gets disabled), while a miss is survivable. That single classification settled three separate calls: fix the #1248 self-trip, decline to chase [#876](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/876)/[#817](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/817) by widening markers (a denylist widen that never converges, per [#1150](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1150)), and - answering the bot review - *trim* two speculative verbs rather than write tests that would cement vocabulary I invented.

**The first live run surfaced a governance question I deliberately did not answer.** It flagged [#1147](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1147) and [#659](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/659) in Scientist's Ready lane. Against the DoR as written ("no unresolved decision inside an AC, no matter how small it looks") both are correct - but #659's "Determine whether ImmunoStruct accepts wild-type-free inputs" **is** the research question, not a blocker on starting. So either they are genuine miscommits, or the DoR needs a research/eval carve-out.
I implemented the convention as written rather than silently narrowing it in code. **A mechanism that faithfully implements a rule is the fastest way to discover the rule's edges** - and when it does, the fix belongs in the rule first and the code second, or the code quietly becomes the real policy. Left open on the PR for pushback.

**Also carved [#1291](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1291)**, closing the [#1207](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1207)->[#681](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/681) leaf-routing I had owed since 07-22. No open leaf fit ([#1175](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1175) right shape but closed; [#1176](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1176) a different axis; [#1177](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1177) blocked), so carve beat reuse. Kept the 54-confident / 28-low-confidence split explicit in the body rather than folding one batch of 82 - Scientist's post-main-text correction downgraded the ovarian set, and that nuance would otherwise have survived only in a comment thread the registry's future reader will never see.

**And answered a scoping question with "no" before doing the work.** Asked whether these two could run as a quick-win burn-down: the [#1207](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1207) routing produces an Issue, not a mergeable diff, and PM's best-shaped quick win ([#1244](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1244)) targets a personas-repo file PM cannot commit, so it structurally cannot park at In review. **PM's lane may not support the burn-down pattern at all**, because its quick wins are memory/governance edits that route through MM's commit lifecycle - worth its own Issue if the pattern is wanted here.
...
**The freshness check earned its keep by catching a pull that was already shipped, and the near-miss and its root cause are the same recurring cross-repo gap.** I pulled [Issue #1156](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1156) from Ready and, before editing, verified the live tree per the freshness rule. All five ACs were already satisfied: the memory reconciliation had landed 2026-07-20 via a *parallel* personas PR ([#178](https://github.com/Jin-HoMLee/claude-personas-splice-neoepitope-pipeline/pull/178) closing personas #177), the AGENTS.md merge-time framing arrived with [Issue #1155](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1155) itself, and the open-parent Priority sweep had run. I nearly spent a committed pull slot re-implementing shipped work. The root cause is a cross-repo close-routing gap: a personas PR satisfied a project Issue's ACs without wiring `Closes ...#1156`, so #1156 lingered Open with every AC met. Third sighting, so it now has a carrier ([Issue #1260](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1260)) instead of a post-it watch line. Closing #1156 surfaced completion-drift on parent [Issue #1031](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1031) (both children COMPLETED, own ACs ticked), rolled up in the same pass.

**The warm-up ([Issue #1223](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1223) -> [PR #1261](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1261)) shipped clean, and the bot review taught me where the tests actually live.** The change threads `mode` into the SDR seed so window mode forces a board-carrier decision per Carried-forward / Retrospective item (else it dead-letters in the read-only artifact); milestone mode is untouched, pinned by a matched-pair test. The LGTM review's two useful findings both landed on the tests: I had put the new tests in `workflow/tests/` while every other `seed_narrative` test lives in `tools/ci/test_milestone_report.py::TestSeedNarrative` (a different CI job, with a duplicated bootstrap) - my grep had only covered `workflow/tests/`; and my window-mode assertion was under-constrained (both forcing blocks share the same substrings, so dropping one section still passed). Relocated + strengthened to `count("observation only, no carrier") == 2`, and verified the falsifier fails when a section is removed. "A check must be able to fail", again, and from the outside this time.

**Cross-repo, again: AC2 straddles both repos, so the close routing is a live decision at the merge gate.** AC2's README half is in this project PR; its Friday-cleanup memory mirror (`feedback_morning_routine.md`) is a personas edit MM drains. So #1223 cannot cleanly close from the project PR alone until the memory half lands - the same shape [Issue #1260](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1260) exists to settle. At the merge gate now; Jin-Ho's call, plus the A/B close-routing choice.
...
**The finding that matters most is one I caused.** Selected [Issue #876](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/876) as best-next, ran the pre-start freshness check, and it disqualified the Issue. Jin-Ho's 2026-07-04 verdict had **trigger-gated** it (*"not building now; revisit when a scheduled HTML post or durable archive is genuinely needed"*) and ordered its ACs rewritten to the narrowed scope. Neither happened - **and I committed it to Ready the next day anyway**, writing *"DoR met: scope is refined"*, a claim a one-minute body-read falsifies. Un-committed it back to Backlog.

The sharp part is not the wrong commit; it is what would have happened had I not checked. **The ACs still describe the dropped wide scope** (a glanceable phase rail replacing the wall of text - which marginalia now owns). I would have built the thing we explicitly decided *not* to build, and **the closure ritual would have passed me with every box honestly ticked.** That is `feedback_ac_gates_intent_not_proxy` firing on live work: the gate cannot catch it, because the boxes are true.

**Third instance of the trigger-gated class** ([#817](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/817), [#601](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/601), now #876), and the first where I both **set** the gate and **walked through** it. A trigger-gated Issue has no `blockedBy` edge and no board field, so it scans as clean and pullable forever; the gate lives only in a comment. **Only a body-read at the start line catches it.** My own post-it had #876 queued as "next pull" for days, confidently, and never re-read the body. The pattern from this whole week completes itself: **the checks that fire are the ones I run at the start line, not the ones I trust from my notes.**

**[Issue #745](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/745): I cut half of it rather than build it.** The Issue bundled a *near-deadline milestone sweep* and a *freshly-unblocked sweep*. Before building, I measured the first one's fuel: only **2 open Issues board-wide carry a milestone**; the one open milestone with a `due_on` has **zero** open issues; the one with issues has **no** `due_on`. Post-[#902](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/902) facet 2 (all-Kanban shop) that is **structural, not a lull**. Building it would have shipped **a check that cannot fail** - the exact defect class I spent this morning cataloguing in others. Cut it on the Issue, in the open, with the numbers.

**The other half was alive, and the design turned on rejecting the obvious rule.** The naive detector - *"unblocked AND still in Backlog"* - would **fight our own model**: under late-commitment Kanban, uncommitted-in-Backlog is the *normal, correct resting state* of an option, so that rule nags forever about items that are exactly where they belong. The real signal is the **state change**: a blocker closed *recently* while the dependent stayed uncommitted. That is [#594](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/594)'s shape exactly (prerequisites cleared, sat un-pulled 4 days).

**It caught a live one on its first real run:** [Issue #911](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/911), in Backlog, blocker [#1086](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1086) closed 2026-07-11, nobody re-pulled it. Verified independently with a separate query rather than trusting the script's own output. Scientist's card - **routed, not pulled**.

**Bot review found a real bug, and I disagreed with its severity in the direction of fixing it.** `blockedBy(first: 20)` was a **sample**, not a bound (GitHub's documented ceiling is 50, which the sibling `scan_prose_deps` cites). A cap is not a filter and GraphQL does not guarantee node order, so an issue could return 20 all-CLOSED blockers while a still-OPEN one sorted past the cap -> **a false "freshly-unblocked" finding on a genuinely blocked Issue.** The bot graded it low-severity on blast radius (we carry 0-2 blockers). I graded it by **kind**: that output feeds *"never commit a blocked Issue to Ready"* the precise input that rule exists to prevent. Free to fix, so the practical-risk argument never needed adjudicating. Pinned by a test.

**On my own checks:** 29 tests, built as **matched pairs** (blocker state / clear age / board Status - same payload, one variable flipped, opposite outcomes). I did not assume they could fail: I disabled the age window in the source, watched exactly 3 go red, and reverted. Then re-ran the **live smoke** after the query change, because a green unit test says nothing about the real API.

**Also landed:** [#1169](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1169)'s three scope calls (all four fold-in candidates out; two rules minted - *a trigger-gated Issue must never be an epic child*, and *size is not a scope test*), which exposed that the epic's **goal sentence was broader than its membership** and would have let the bar read 100% with a named part of the goal outside it (the [#859](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/859) false-bar shape). Narrowed it. [#1095](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1095) closed NOT_PLANNED with its revival trigger given an **open carrier** on [#1116](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1116) - a note on a closed Issue is invisible to every queue. [#1150](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1150) triaged and **lifted P2 -> P1**: it blocks [#1094](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1094) + [#1083](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1083), a dependency that lived only in prose until I wired the native edges.
...
**What.** The `arc-phase:active` focus slate was hard-capped at 3 arcs. It is now an advisory `~3` guideline that prints a `NOTE` and never gates. `arc:immunogenicity-benchmark` promoted to `active`; the slate is deliberately **4**, and nothing was demoted.

**How it surfaced.** Settling [Discussion #949](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/discussions/949) (Scientist's portfolio-broadening request) required promoting an arc, and the cap said the slate was full. I built the demotion of `arc:board-governance` into a recommendation, with a rationale that read fine: "zero In-progress WIP, so nothing is stranded." Jin-Ho asked whether hard-capping active arcs was still best practice. It was not.

**What the data said.** Closed issues 2026-06-19..07-10, against each arc's phase: `board-governance` (active) 61, un-arced 35, `immunogenicity-benchmark` (**next**) 16, `scoring-tcr-pmhc` (active) 13, `memory-methodology` (**later**) 10, `cloud-reproducibility` (next) 10, `junction-filtering` (later) 3, `aligner-junctions` (active) 3.

A `later` arc - which our own memory defines as "parked, we do not intend to pull this now" - out-delivered two of three `active` arcs. Roughly a quarter of closed work carried no `arc:` label at all. The board's only In-progress item sat in a `next` arc. **`active` does not predict throughput.** My "zero In-progress WIP" rationale was true and irrelevant: `board-governance` was the single highest-throughput arc on the board.

**What the web said.** WIP limits attach to work in flight, not to categories. SAFe's portfolio WIP limit counts epics moving through Kanban states ([agility-at-scale](https://agility-at-scale.com/safe/lpm/work-in-progress/), fetched directly); strategic themes, the tier our arcs occupy, are not WIP-limited in any source I could reach. Two of four candidate pages returned nothing substantive and one was behind a login wall, so this rests on one solid fetch plus a weak corroboration - recorded as such rather than dressed up. We already run three real WIP limits, all counting items, all advisory. The arc cap was the only blocking guard and the only one without an analogue.

**Lesson (the reason this entry exists).** The cap turned a healthy state - four arcs worth attention - into a "violation," and I was one command from demoting the busiest arc to satisfy a counter, for zero behavior change. `feedback_rule_as_suspect.md` names this exactly: a correctly-followed rule is a candidate defect; fix the counter, not the label. I did not catch it myself. What I had was the *unease*, and I spent it writing a persuasive rationale instead of auditing the premise. **The tell was there: I was about to demote something productive for no observable reason.** That feeling is the signal, and next time it should route to `feedback_verify_premise_before_mechanizing.md` before it routes to prose.

This is the **second** under-defended claim found in `shared/feedback_arc_review.md` in two days - the first being its "themes are flat labels" citation, retracted in #949 by checking Cohn directly. Both were rules I authored or relied on. The file is flagged for a whole-reasoning review rather than a third patch.

**Two mistakes worth recording.**
1. I asserted mid-session that multi-arc did not affect the exit code, from a `grep -A3` whose context window began one line *below* the `drift=1`. Running `--check` disproved it. Consequence is real: `--check` is now permanently red on #1036, since it is the only remaining finding, and greening it under the current rule would require exactly the sweep the Scientist warned against. Carried by [#1103](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1103).
2. I edited `apply_arc_labels.sh` without grepping for its tests. `test_more_than_three_active_flagged` asserted the old contract and CI went red. Caught from the check rollup, not from having looked. My Test plan had ticked boxes for hand-run behavior while never running the suite that covered the change.

**On the review.** The bot's blocking finding was that same stale test, already fixed by the time the verdict landed; it independently proposed the `ACTIVE_SLATE_GUIDELINE=99` suppression case I had added. Its non-blocking claim that a non-numeric knob "aborts the whole reconcile" under `set -euo pipefail` is **wrong** - an `if` condition is exempt from `set -e`; probed it directly, execution continues and exits 0. The real defect (cryptic stderr, silently ignored knob) was still worth fixing, so the fix landed and the reasoning was corrected on the PR. Verified rather than implemented on trust.

**Scientist's #1036 retraction: counted.** The `Watch-for (n=1)` note in `feedback_arc_review.md` has genuinely reached **n=2** (#678, then parent #1036, verified as a real parent with 2 sub-issues). Recorded in the memory file, not just in the thread.

**Side effects applied.** #1045/#1048/#1096/#1097 arc-labelled `immunogenicity-benchmark` (the four the Scientist left un-arced pending this call); reconcile cleared 16 stale `arc-phase` labels; pinned slate card refreshed; #949 answered and left open for the raiser to close.

**Not done, deliberately.** #1045 was **not** converted into the initiative epic. The Scientist and I agreed the shape, but adopting #685/#699/#684 as children is a structural change neither of us ran past Jin-Ho, and ratifying a decision is not authorizing the action (`feedback_decision_ratification_not_action_authorization.md`).

**State.** At the merge gate, awaiting Jin-Ho. Companion `shared/feedback_arc_review.md` edit is on disk in the personas clone for MM to commit.
