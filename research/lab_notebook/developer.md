...
**Headline:** The deck was CI-green, bot-reviewed, and overflow-scanned yesterday - "at the merge gate." Jin-Ho's manual review of slide 4 ("The instrument") found four successive accuracy defects that every automated check had passed, and fixing the last one required measuring our own output and discovering the schematic depicted a scenario that never occurs. Merged after the fixes; two follow-ups filed.

**The four slide-4 catches, each deeper than the last.** (1) The schematic claimed frames 1-2 have stop codons but never *drew* them - a stale generator comment said "with stop markers" that the code didn't back. Added red `*` cells. (2) The headline said "keep **the single** breakpoint-crossing stretch," but `orf_fasta_from_contigs.py` loops all three frames and emits every crossing stretch (up to 3/contig) - reworded to "each frame's." (3) Jin-Ho asked whether stops are really that abundant. They are not: I measured the actual run (`combined.fasta`, 335 stretches over 122 junctions) and found **mean 2.74/3 frames kept, 75% of junctions keep all 3, none keep just one**. My figure drew 2 stops/frame and showed one-frame-wins/two-drop - a case that **literally never happened in the run**. Redrew a representative sparse-stop contig (2 kept, 1 dropped as the labeled minority), caption carrying the real numbers. (4) Also replaced slide 6's mermaid fan-out (its JS-measured node boxes overflowed their text in reveal.js) with a deterministic matplotlib figure - the deck is now uniformly real-artifact, no mermaid, which is the rule I should have applied to that slide the first time.

**The lesson that stings: a green board is not a correct one.** CI dry-run never executes the emitter, the unit fixtures are strand `+` only, the bot review read the prose not the run data, and my own overflow scan checked layout not truth. The single check that caught the real defect was a human asking "is that realistic?" and me then going to the data instead of defending the drawing. The `state-the-predicted-effect-size` reflex is what closed it - I didn't argue the figure was fine, I computed what "fine" would look like (≈1 stop/frame random, ≈0 in the true frame) and the drawing failed it.

**Two follow-ups, correctly routed.** The 2.74/3 measurement exposed that ~60% of emitted stretches are wrong-frame translations padding the MS search DB - a methods/interpretation question, so filed for the Scientist as [#1283](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1283) with the data and role-respecting ACs (I report the measurement; Sci decides the policy). Separately, a parked adversarial bug-hunt surfaced and I independently code-verified a genuine minus-strand arm-swap bug in `assemble_contigs.py:281` (fabricates the junction for ~half of junctions, invisible to `+`-only tests) - filed as [#1278](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1278), not started.

**Process:** verified Codex's interrupted-session wind-down -> independently confirmed the #1278 lead -> per-slide review loop with Jin-Ho (fix -> re-render -> headless-Chrome print-pdf scan -> show, before each commit) -> filed #1283 -> merge gate. `project-map-freshness` failed once with an 86-line drift I could not reproduce locally; the tell was CI-drift-with-zero-local-drift, which means the branch was behind main and CI checks the PR-merge tree (captured as [[atlas-freshness-checks-pr-merge-tree]]). Merged origin/main, regenerated the atlas over the merged tree, stripped the `project_map.html` shell conflict markers. All 6 checks green; ACs ticked and merge on Jin-Ho's explicit "yes to both."

## 2026-07-21

### 10:33 UTC - Editor: Developer - An experiment-tier deck for the junction-ORF method, and figures that are real artifacts rather than drawings ([PR #1263](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1263) for [Issue #1262](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1262))

**Headline:** Jin-Ho called the junction-ORF work an experiment and asked for a slide deck. Built a 9-slide experiment-tier Quarto deck (`research/experiments/issue_1204_orf_fasta/slides.qmd`) and filed/tracked it as its own Issue rather than bolting it onto the merged [Issue #1204](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1204) PR - the deck references the *merged* emitter, so figures regenerate against the final tool.

**The scope call that keeps the deck honest:** this is the *method + instrument + validation* chapter, explicitly **not** the results chapter. The tempting overclaim - "we found junction neoantigens" - is false; all we have is that the search-DB instrument works and recovers a peptide we planted. So the framing throughout is "prediction is a hypothesis, not evidence" and "recovery is the check, not significance," with the actual re-search deferred to [Issue #1176](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1176) AC-6. The bot review singled this hedging out as "exactly right for a smoke-tier artifact," which is the reassurance I wanted that the honesty reads as honesty and not timidity.

**Real artifacts over drawings.** Three figures, all offline-regenerable from committed inputs (`figures/_regenerate_figures.py` + `outputs/orf_run_summary.json` + the cached `snakemake --rulegraph`): the *actual* pipeline rulegraph (the two new rules recolored gold so "where it plugs in" is the real DAG, not a hand-drawn approximation), a matplotlib ORF-stretch schematic, and the annotated b/y recovery spectrum for `LGLQVPTTTLG` - the money shot, since it shows a genuine junction peptide matched rather than a summary statistic. The b/y colors reuse the project blue/gold, which I ran through the dataviz palette validator (CVD dE 28) before shipping rather than assuming a two-color pair is safe. Mermaid stayed only for the one genuinely conceptual/future diagram (the superset-representation projections), which is the sanctioned use.

**Process:** the deck upgrade to the smoke (recovery, not just loadability) happened first at Jin-Ho's request on [PR #1259](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1259); this deck consumes that recovery result. Overflow-checked all 9 slides via a headless-Chrome print-pdf scan (reveal.js clips silently). One cross-role AC (Scientist review of the biology-framing slides, sourced from the run4 brief) is pinged to Sci and can't be self-signed - I'll raise the merge-gate choice (ship on Dev ACs vs hold for Sci) with the human. Stopping at the `gh pr merge` command; AC ticks need authorization (auto-mode-classifier-blocked).

**Headline:** Built the genome-wide junction-spanning ORF-FASTA emitter that unblocks the [Issue #1176](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1176) MS re-search (AC-6). It reuses `assemble_contigs.py` at 30-nt flanks and adds `orf_fasta_from_contigs.py`: translate all 3 frames of the 60-nt contig, split at stop codons, keep the single stretch that crosses the breakpoint (`nt_start < 30 < nt_end`), emit as a FASTA mini-protein `junction_id|frame|chrom:start-end:strand`. Wired as an optional Snakemake rule (`ms_search_db.smk`); the production MHCflurry k-mer path is untouched. Full superpowers flow: brainstorm -> spec -> plan -> subagent-driven execution (6 tasks) -> final review -> bot review.

**Why a new emitter and not a flag on `translate_peptides.py` (Sci's spec call, and it is right for a deeper reason than format).** The k-mer path emits fixed-length 8/9/10-mers as TSV for MHCflurry; an MS engine wants variable-length stop-to-stop ORF stretches as FASTA and does its own digestion. That is the surface reason. The deeper one, which surfaced when Jin-Ho asked whether the FASTA step should become a pipeline stage: **the ORF-stretch FASTA is a superset representation - you can derive any k-mer set (class-I 8-11, class-II 13-25) by substringing a stretch, but never a stretch from k-mers.** Today the pipeline bakes two consumer-specific decisions upstream (the 27-nt flank caps class-I at 10-mers in `assemble_contigs`; fixed windowing lives in `translate_peptides`). The clean future architecture makes the ORF stretch the canonical intermediate with MHC-I / MHC-II / MS as projections - and two of those three consumers (MHC-II predictors, MS engines) take protein FASTA directly; only MHCflurry-I insists on pre-cut k-mers. I did **not** fold that refactor into this Issue (it touches the green production DAG and only pays off once MHC-II is real), and filed it as trigger-gated [Issue #1255](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1255) instead, writing this emitter's internals (translate + stop-split as consumer-agnostic functions) to be *promotable* into that intermediate as a wiring change. The vision reinforced the surgical choice (Fork 1-A decoupled 30-nt flank) over a global `peptide_lengths` bump: the stretch dissolves the flank-width question entirely.

**AC-3 verified with a real $0 live smoke, not a format assertion.** The verification could have been structural-only (does the FASTA parse), but the live-integration-smoke rule says a boundary-touching change (producing input for an external engine) needs one real run. Sage is the local license-clean engine (MIT, native aarch64-apple-darwin, CPU) the [Issue #1176](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1176) run4 brief already selected, so I pulled the v0.14.7 macOS-arm64 release binary and ran the smoke: the 336-entry combined FASTA (canonical + 335 junction ORF from the chr22 integration) **loaded and nonspecifically digested to 694,494 fragments / 49,432 peptides + decoys, search exit 0, no parse error**. Zero PSMs is expected and fine - the synthetic 8-peak spectrum is below Sage's `min_peaks` default and a hit was explicitly a bonus; loadability + digestion + clean completion is the deliverable, and real recovery is [Issue #1176](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1176) AC-6. The chr22 integration also exercised the execute-time-only paths that `snakemake -n` cannot see (conda env build, the `stats=None` branch, the biopython import) - 335 records, 0 stop/X, all-60-nt contigs, headers unique - which is exactly the class of bug my integration-run-before-new-rules rule exists for.

**Process notes worth keeping.** (1) Subagent-driven execution held its quality bar: the task reviewers caught two real gaps a green suite would have hidden - a `crossing_orf_stretch` signature that dropped its spec'd type hints, and a test *named* `..._drops_x_and_short_...` that never actually exercised the min-len drop path (a check that could not fail on the thing it existed to catch). Both fixed, the second with a confirmed falsifier (writing `KKKK` if the filter were removed). (2) The final whole-branch review (opus) and the bot review independently traced the boundary math and falsifiers and both returned no blockers; the bot's three nits (dead `_codon_seq` helper, opaque `IndexError` on a malformed header, header-vs-sequence dedup) were all cheap and sound, addressed in `ad21649`.

**Process:** gogo -> freshness-check + In progress -> brainstorm (2 forks resolved) -> spec `6f3286c` -> plan `12bfb39` -> subagent-driven 6 tasks (2 fix loops) -> final review clean -> chr22 integration + 5-dir sweep (2061) + Sage smoke -> PR [#1259](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1259) -> auto-bot-review (no blockers) -> nits addressed `ad21649` -> reviewer reply -> this entry -> merge gate. Stopping at the `gh pr merge` command for Jin-Ho; the [Issue #1204](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1204) AC-box ticks there need his authorization (auto-mode-classifier-blocked).

## 2026-07-20

### 19:44 UTC - Editor: Developer - The lightning_sdk runner lands, and a flaky link taught the difference between a live process and a progressing one ([PR #1253](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1253) for [Issue #1249](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1249))

**Headline:** Finished the hands-free `tools/lightning/run_tfold.py` runner (all 6 ACs). The one open AC was the persistence falsifier, and it holds: pre-staged the 2.43 GB ESM-650M weights to `TORCH_HOME=$HOME/.torch`, then re-ran `prestage` *after a full Studio stop* - the cache hit in ~4 min (not the ~20-min re-download) with the four `.pth` files carrying their **original** download timestamps. Timestamps, not just presence, are the discriminator: a re-fetch would restamp them, so the untouched mtimes prove survival across the stop rather than a silent re-download that would also leave four files sitting there.

**The falsifier had to distinguish two outcomes that produce the same end state.** "Weights are in the cache after a stop/start" is true whether they *survived* or were *re-downloaded* - both leave four complete files. A check that only lists the files can't fail on the bad outcome. What separates them is the *cost signature*: ~4 min + unchanged mtimes + zero download progress = survived; ~20 min + fresh mtimes + a 3 GB transfer = re-fetched. I built the check to key on the signature, not the presence. Same shape as the recurring THE-ONE-THING note (a check must be able to fail on the thing it exists to catch).

**A flaky connection wedged the smoke for 87 minutes, and "the process is alive" hid it.** After the weights landed I launched the `$0` CPU smoke; my link dropped mid-`studio.start()` and the SDK call blocked on a dead socket. The local process stayed *alive* (`ps` showed it running) and its last line was `starting on CPU ...` - so by the naive signal it was "still working." It was not: `ETIME` was 1h27m for a step that takes ~3 min, and Jin-Ho's Lightning dashboard independently showed the Studio **sleeping** the whole time. Two external signals (elapsed-time sanity + the dashboard) exposed what the in-process signal could not. Killed it; a fresh status probe confirmed the Studio Stopped (CPU, so $0 wasted). **Fix carried forward: every remote Studio call now runs under an 8-min watchdog that hard-kills on timeout** - a wedged call can no longer masquerade as a running one. Lesson: a live PID is not a progressing task; when driving a remote thing over an unreliable link, bound it and cross-check against an out-of-band signal (elapsed time, the provider's own dashboard), never just "is my process still up."

**Review (auto-bot, 5m11s, no blockers) - 5 findings, all addressed in `b9b826a`.** The one worth recording is #1: `predict --gpu` against an *already-running* CPU Studio would silently run ~79 min on free CPU because `_ensure_running` only sets the machine on the *stopped* path. The bot offered "switch or abort"; I chose **abort**, because `switch_machine` on the already-running path leaves `started=False`, so `cmd_predict`'s `finally`-stop never fires and the switched-to L40S would bill idle - the auto-switch fix would have introduced the exact never-leave-a-GPU-running hazard the runner exists to prevent. The safe realization of a review suggestion was the *narrower* one. The `predict` path itself (findings #1/#2) is not live-exercised - it needs the L40S, which is Sci-owned #1245 - so it is verified by reading + a local `_same_machine` pure-function check, and the $0 smoke is re-verified green post-edit.

**Process:** resume -> finish AC2 (prestage + falsifier) -> smoke (AC6) -> PR #1253 (auto-bot-review on open) -> review -> fixes (`b9b826a`) -> reviewer reply -> this entry -> merge gate. Stopping at the `gh pr merge` command for Jin-Ho's final look; the AC-box tick there needs his authorization (auto-mode-classifier-blocked).
...
Backfilled the stale project-map atlas (`graph.json` had drifted to 335 nodes vs 678 fresh) and added an always-on `project-map-freshness` CI gate that regenerates both artifacts and fails on any diff. Two bugs surfaced on the way, one I caught and one I did not.

The one I caught: `build_html.py` was non-idempotent - it netted one extra blank line after `<head>` on every rebuild (840 -> 841 -> 842 lines), because the D3-strip regex removed the block's *trailing* newline but not the *leading* one `build()` inserts via `"<head>\n" + block`. A `git diff --exit-code` gate is impossible on a builder whose output never stabilizes, so this was a genuine precondition, not a nice-to-have. Fixed by making the strip the exact inverse of the insert; three consecutive builds now hash identically.

The one I missed, and it is the interesting one: I "verified" AC3 (the gate greens on a fresh artifact) with a same-machine idempotency check - regenerate, `git diff --quiet`, exit 0. Green. But the gate's real contract is that the *committed* artifact byte-matches a *fresh CI regen on a different filesystem*, and I never tested that axis. The first-pass bot review did: it read the PR's own red CI, spotted that insertions exactly equalled deletions with an unchanged 678-node/854-edge count (the fingerprint of a pure reordering, not content drift), and traced it to `os.walk`/`rglob` appending in `os.scandir` order - unsorted and not portable across filesystems. My machine and the ext4 runner enumerate directories differently, so the committed copy could never match a CI regen and the gate was unsatisfiable as shipped. Fixed with a total final sort in `build_graph()` (nodes by id, edges by source/target/type, both unique post-dedup), regenerated, and this time confirmed the gate green *in CI* (run 29608480036, all six jobs) - not just locally.

What I make of it: my verification tested the wrong invariant. "Idempotent on my machine" and "portable across machines" look like the same property until a sorted-vs-scandir gap splits them, and the whole point of a regenerate-in-CI gate is the *second* property - so the check I ran could not have failed on the bug the feature exists to prevent. This is `a-check-must-be-able-to-fail` in a new dress: same-machine idempotency is structurally blind to filesystem-order drift, exactly as a synthetic payload is blind to the real trigger. The concrete rule I am taking: **when a gate's contract is cross-environment (CI regen vs committed), the acceptance check must exercise the cross-environment axis - reordering the enumeration, a second machine, or at minimum letting CI itself be the falsifier before ticking the box.** Ticking "gate greens on fresh" off a same-machine run was ticking a box the test could not have reddened. The bot also flagged a real design tradeoff (an always-on whole-tree blob gate invites `graph.json` merge conflicts at this PR velocity); #789 had weighed and rejected the alternatives, so I kept always-on and surfaced the friction as a maintainer follow-up rather than folding a pivot into a backfill PR.

### 14:30 UTC - Editor: Developer - The Created-by footer guard, and a false-positive the bot caught before it could erode trust ([PR #1229](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1229) for [Issue #1094](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1094))

Built the mechanism escalation for the `**Created by:** <Role>` comment footer (slipped >= 3x in one session, twice after being inlined into memory). The design decision worth recording: it composes with the #1197 decision I made an hour earlier rather than contradicting it. #1197 said "don't chase the unbounded Bash-write paths for the Edit/Write em-dash guard." This guard reads a *bounded* `gh comment` command's body, so extracting the heredoc body - the exact path comments are authored with - is the REQUIRED #1130/#1142 "handle the real shape" work, not the forbidden unbounded chase. The two are consistent: parse the one command you're guarding; do not try to parse every way a file could be written. I verified the guard live by driving the real trigger (a footerless `gh pr comment` through the harness got blocked), not a synthetic payload - and it promptly guarded me too.

What I make of it: the bot review found a genuine false-positive I had built in, and false-positives are the one failure direction that kills a guard. `extract_body` bound `--body-file -` to the *first* heredoc; in a chained `cat > scratch <<EOF ... EOF; gh comment --body-file - <<BODY ... BODY` the first heredoc is the scratch file, so a *compliant* comment (footer in its own BODY heredoc) would be denied. That is precisely the "over-eager guard is worse than none" failure the repo's own guard philosophy warns about - I wrote the guard citing that philosophy and then planted an instance of it. The fix is the fail-open direction the rest of the hook already takes: >1 heredoc is ambiguous, so allow. It cost two lines and a test. The lesson is not new (drive-the-real-trigger, a-check-must-be-able-to-fail), but the shape is worth keeping: when a guard has multiple ways to acquire its input, each acquisition path needs its own falsifier, because the *common* path (single heredoc) tested green and hid the *chained* path entirely.

Also closed the sequencing loop I had flagged: merged main (with #1197's now-landed note) into this branch and refreshed the note's "Created by: footer not yet built" line to "now built", so the two PRs do not leave a stale cross-reference. Promoted `_shell_parse._HEREDOC_RE` to public (`HEREDOC_RE`) since this hook is now a second, cross-module consumer of it (the bot's nit 3).

### 13:20 UTC - Editor: Developer - Settling the heredoc-bypass decision, and echoing an unverified premise into a decision record ([PR #1228](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1228) for [Issue #1197](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1197))

Ratified #1197: the content-guard family's `Bash`-write blind spot is **accepted and documented**, not closed by widening matchers. The reasoning is the interesting part and it held up under a web cross-check: extending a guard to parse `cat > f <<EOF` bodies is a denylist widen (CWE-184) over an unbounded set of Bash write paths (`printf`/`echo`/`tee`/`python -c`/redirection all remain), so it never converges and manufactures false confidence; the convergent fix is at the **effect level** (inspect file content/diff after any write, tool-agnostic - OWASP defense-in-depth's "even if input detection fails, output validation can catch"), which I filed deferred as [#1227](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1227). One decision cleared the boundary case, unblocked #1094, and spun out the convergent follow-up. I surfaced the honest tension too (the real threat model is accidental self-bypass, where heredoc is a common path, so "extend" has more value than pure denylist-futility admits) rather than only the conclusion.

What I make of it: the recurring miss struck again, and this time inside the deliverable itself. The PR was a *decision record*, so accuracy was the whole product - and I wrote "the content guards (no-em-dash, @-claude-mention, Created by: footer) are bound to the Edit/Write matchers," which the bot review checked against `.agents/settings.json` and found false: only `check_no_emdash` is Edit/Write-bound. The `@-claude` guard is `Bash`-bound (its blind spot is the command-string #1142 one, a *different* family), and the `Created by:` guard does not exist (it is a manual step; #1094 would build it, and it would be Bash-bound too). I had copied the premise straight from #1197's own problem statement without opening the two hook files - the exact `verify-location-facts-against-source` / `verify-premise-before-mechanizing` failure, and the eighth session where an external probe finds the precision-class thing my own read glides over. That a *canonical governance doc* was the thing I got loosely-sourced is the sharp edge: the cost of an unverified premise scales with how authoritative the artifact is, and a decision record is maximally authoritative.

Lesson (sharpening, not a new rule): when the artifact IS a claim about how the system is wired - a decision record, an ADR, a "here is how X works" doc - the premise is the deliverable, so open the file/settings and confirm each named binding *before* writing the sentence, not after a reviewer asks. A PM-authored premise inherited into my doc is still my claim once I ship it. Corrected the AGENTS.md note, #1227's framing, and posted a scope-correction on #1197 so the record is not left inaccurate.

### 12:50 UTC - Editor: Developer - Moving the parent guard branch-time to merge-time, and the false-PASS my own re-read missed again ([PR #1224](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/pull/1224) for [Issue #1155](https://github.com/Jin-HoMLee/splice-neoepitope-pipeline/issues/1155))

Resumed #1155 at the TDD build (design settled last session). The change is a reframing: the PR #543 -> #538 harm (a PR auto-closing a parent and orphaning its open children) lands *at merge*, not at branch, so the branch-time ban was guarding the wrong act. Added a merge-time gate (`tools/ci/parent_child_gate.py`, gate 7 in `audit_and_merge.sh`) that blocks a merge whose PR closes a parent with an open child, and downgraded the two branch-time guards (the `gh issue develop` hook + `new_branch.sh`) to advisory `ask`/warn. Both halves shipped together on purpose - relaxing the branch guard before the merge gate exists would be a no-guard window. The one deliberate divergence is fail-posture B: this gate fails *closed* on an undetermined parent state, unlike every fail-open sibling, because orphaning is irreversible (`feedback_fail_safe_not_fail_open`: a gate must never false-PASS).

What I make of it: the fail-closed posture is the whole spine of the design, and I still built the one code path that violated it. The bot review found it - `_fetch_parent_state` fetched `subIssues(first:100)` with no pagination, so a parent with >100 children whose open ones sit past the first page would classify `clean` and admit the merge. A false-PASS in the gate whose docstring cites "never false-PASS." This is the seventh session running where my own re-read never surfaces the precision/duplicate-site class and an external probe does (post-it's "one thing to carry forward"). The interim habit that would have caught it: for a gate built on an invariant, write the probe that tries to *violate* the invariant - here, "what input makes it return `clean` when it should not." I fixed it with full pagination (chosen over the reviewer's minimal `len < total -> undetermined`, which would permanently block a legitimately-mergeable >100-child epic) plus a defensive `len < total -> undetermined` net on top, and the truncation test drives that net. Finding 2 (over-block when a PR closes a parent and its last child together) was a fail-safe precision miss, fixed by threading the PR's closing set into the decision.

Second thread worth keeping: Jin-Ho asked "is the MM gonna see the flag in the PR body?" about my coordination note. It would not - MM's `/inbox` scans board-wide `To:` comment pings and open Discussions, and a PR *body* is neither. A note there is a silent dead-drop, the same delivery-gap the resume-routine memory names ("a rule that must fire cannot live in a file nobody loads"). Moved it to a `**To:** Memory Manager` comment on #1155 and verified it lands in `scan_addressed_comments.py --role memory_manager`. The lesson generalizes past coordination notes: before treating any written hand-off as delivered, name the channel the recipient actually scans and confirm the artifact is on it.
